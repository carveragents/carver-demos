...
**Issue 2 exposed a defect underneath the one reported.** Fixing the `Verify` table meant asking
...
Three files are written by more than one task: **`scorers.test.ts`** (P6.4 → P6.3 → P6.12b),
**`carverGuardrail.test.ts`** (P6.5 → P6.6 → P6.9 → P6.10) and **`evals.test.ts`** (P6.12a →
P6.12b → P6.14). The rule, which is not optional:
...
- **This task creates the file containing ONLY the `judge_cases` describe and ONLY the imports that
  block needs** (`parseAndValidateVerdicts`, `GuardrailVerdictSchema`, the fixture). It does **not**
  stub out the other three groups. P6.3 and P6.12b add theirs, with their imports, when their
  targets exist — see §0's shared-test-file rule. The bare file run is green here and at every
  later point; the filter proves *this* group specifically.
...
- **Spec:** §12, §4 | **Depends on** (imports only, each naming the symbol — §8's pinned row):
  **P6.12a** (`DeliveryResult` — §8's row for this module *does* list `evals/deliveryWorkflow.ts`,
  unlike P6.12a's non-dependency on agents), **P6.6** (`runJudge`), **P6.4**
  (`GuardrailVerdictSchema`, `parseAndValidateVerdicts`, `JudgeObligationInput`), **P6.7**
  (`narrowObligationsPure`), **P6.3** (`predictsStageAViolation`, `FirmProfileSchema`).
  Plus `@mastra/core/evals`.
