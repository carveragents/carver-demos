...
**Stage:** 02-plan **Round:** 3 (revision)
**Status:** Draft
...
P0.0 branch ─► P0 scaffolding ──┬────────────────────────────────────────────────┐
   ...
   P3.2 ──────────────────────────────────┐  │        P1.9 fixtures ───────┘     │
                                          ▼  │                                   │
                            P6  template/ ◄──┴───────────────────────────────────┘
                              P6.1 synthetic fixtures (cleared-set + ScenarioDecision)
                              → P6.2 GENERATE the 4 fragments (needs P3.2)
                              → P6.3 schema/config/firmProfile → P6.4 judge/contract
                              → P6.5 sharedConfig+baseline+judge → P6.6 callJudge
                              → P6.7 narrowing → P6.8 containment → P6.9 CarverGuardrail
                              → P6.10 guardedAgent → P6.11 workflows → P6.12 evals
                              → P6.13 prompts.test → P6.14 report/mastra/scripts
                              → P6.15 drift → P6.16 README → P6.17 gate
                                             │
                            P5 ∧ P6 ─────────▼
...
rejects exactly that: P6.2 runs `prep/`'s generator; P1.1's cutoff test calls P1.4's function; P6.14
...
| **B — `template/` modules** | P6.1 → P6.2 → … → P6.17 | `prep/` **implementation** | **Needs P3.2** before **P6.2** (the generator). B's *module* work can start at P0; its **generated-source gate (P6.2) waits on P3.2** |
