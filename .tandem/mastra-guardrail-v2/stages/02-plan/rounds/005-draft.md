...
A task states an explicit `Verify` **only** when it deviates from that default. The deviations,
named exhaustively so the absence of a `Verify` line is never ambiguous:

| Task | Why the default does not apply |
|---|---|
| **P0.0–P0.5** | Scaffolding — no test file yet; each states its own command. |
| **P6.2** | Runs a generator, not a test suite; verified by the files it wrote. |
| **P6.3, P6.4** | Their cases live in `scorers.test.ts` alongside groups whose targets are not built until **P6.12b**, so the bare file run is legitimately red; both filter with `-t '<group>'`. |
| **P6.5** | Its test file (`carverGuardrail.test.ts`) is only partly landed here; filtered. |
| **P6.11, P6.12a, P6.12b** | Their files are **excluded from `test:unit` at file level**; they need the `-t 'unit:'` selector, never the bare file run. See Phase 6's header. |
| **P3.1–P3.3, P4.1–P4.2** | Stated explicitly (they predate this default in the plan's own reading order; the command is the default's shape). |
| **P6.17, P5.1–P5.2, P7.x, P8.x, P9.x** | Phase gates and live runs; each states its own. |

Every other task takes the default. `P1.2–P1.9`, `P1.12`, `P2.1–P2.4`, `P6.1`, `P6.6`–`P6.10`,
`P6.13`–`P6.16` are the tasks this covers; substitute the task's own module/test name.

**A task is red until its own `Verify` is green — but a *file* being red is not a task being red.**
Several template tasks land cases into a file another task completes (`scorers.test.ts`,
`carverGuardrail.test.ts`, `evals.test.ts`). That is ordinary TDD across a module boundary, and it
is why those tasks filter. **Phase 6's gate (P6.17) is where every file must be green at once** —
nothing ships on a filtered pass.
...
- **This task creates `scorers.test.ts`, and pins its structure:** **one `describe` per golden
  group, named exactly after the group** — `describe("judge_cases", …)`,
  `describe("citation_date_cases", …)`, `describe("obligation_cases", …)`,
  `describe("stage_a_predicate_cases", …)`. Three of those four target functions that do not exist
  until P6.3/P6.12b, so this naming is what lets each owning task verify **its own** group with
  `-t '<group>'` instead of running a file that is legitimately red until Phase 6 completes. The
  spec pins no literal test name in this file (unlike `evals.test.ts`), so the convention is free.
- **Verify (deviates from the default):**
  `cd template && npx vitest run tests/scorers.test.ts -t 'judge_cases'` — the bare file run is
  **red until P6.12b** by design; `scoreCitation`/`scoreMissedObligation`/`predictsStageAViolation`
  are not built yet. The whole file is green at **P6.17**'s gate, not here.
...
- **Spec:** §12 | **Depends on:** P6.10 (`guardedAgent`), P6.3 (`schema.ts`)
...
- **Spec:** §12, §4 | **Depends on:** P6.12a (`DeliveryResult`), P6.6 (`runJudge`), P6.7
  (`narrowObligationsPure`), P6.3 (`predictsStageAViolation`)
...
- **Tests first — golden parity** (`scorers.test.ts`, inside `describe("unit: scorers")`), the
  cross-seam parity that is the whole point of the byte-identical fixtures (G8):
  **`citation_date_cases`** → `scoreCitation` / `scoreComplianceDate`;
  **`obligation_cases`** → `scoreMissedObligation` (**every** case, minus those marked
  `prep_only`). Both groups are asserted in `prep/`'s `test_scoring.py` against the Python
  originals (P1.x) and here against these TS ports, over the **same** `scoring_golden.json` — that
  identity is the parity claim. (`judge_cases` → `parseAndValidateVerdicts` is P6.4;
  `stage_a_predicate_cases` → `predictsStageAViolation` is P6.3 — see those tasks.)
...
- **Do:** run the gate. It **exits non-zero** below the threshold:
  ```
  cd prep && .venv/bin/python -c "
  import json, sys
  n = len(json.load(open('data/cleared/cleared_records.json')))
  print(f'cleared survivors: {n}')
  sys.exit(0 if n >= 20 else 3)"
  ```
- **Verify:** the command above exits **0**. **Exit 3 is a HARD STOP** — not a warning, not a
  number to note in a summary and move past. On exit 3 the executing agent **must not run P8.1**;
  it stops and reports to the user per R7.5.
- **The only way past a non-zero exit is a recorded human decision.** If the user, having been
  shown the true number, elects to ship the smaller set, record that decision in the run log
  (`data/results/yield-decision.md`: the count, the `stop_reason`, the survivor breakdown by
  evidence mode, the spend, and the user's explicit instruction to proceed) and re-run this task
  with the threshold the user authorised. **The acknowledgment is a written artifact, not an
  inference.** There is no flag, no env var and no config key that silences this gate — adding one
  would be the "waiving human review" row of §6's table in a new costume.
- **This gate does not license loosening anything.** Goal #11 still governs the *response*: ship
  smaller, report honestly, **never** loosen the filter, weaken the bar, or pad. The gate's job is
  to make sure a human sees the number — not to make the number bigger.
- **Threshold note (deliberate, not sloppy):** R7.5 says "fewer than **~20**". The command pins
  the tilde to a literal `20`, because a command cannot branch on an approximation. A run yielding
  exactly 20 proceeds; 19 stops. If the orchestrator intends a different literal, it is a
  one-token change here — but it must be **a** literal.
