...
**Stage:** 02-plan **Round:** 2 (revision)
**Status:** Draft
...
| 3 | **A hand-written stand-in `scenario/prompts.ts`** — violating §8's generated-only contract — plus `.skip`ped tests until P8.2 | **P6.0** runs the **real** generator against synthetic data, emitting all four fragments mechanically. **Every skip deleted.** P8.2 reruns the same generator on real data. One generator, two inputs, nothing hand-written |
| 4 | **False parallelism**: "P6 INDEPENDENT of P1–P5" (P6.0 needs P3.2); "all P1 leaves independent" (P1.1's cutoff test calls P1.4) | Graph and table rewritten with the **joins named**: P6.0→P3.2, P1.4→P1.1, P1.9→both tracks, P6.14 writing into `prep/`'s tests |
...
P0.0 branch ─► P0 scaffolding ──┬────────────────────────────────────────────────┐
                                │                                                │
                                ├─ P1  prep LEAF                                 │
                                │    P1.0 test_imports (first — the DAG guard)   │
                                │    P1.2 logging_ · P1.3 reader · P1.5 extract  │  ← mutually
                                │    P1.6 urls · P1.7 sampling · P1.12 openai_   │    independent
                                │    P1.13 prompts/*.md                          │
                                │    P1.4 candidates ─► P1.1 config  (JOIN: config's
                                │                       cutoff test calls assert_cutoff_margin)
                                │    P1.11 budget · P1.8 scenarios               │
                                │    P1.9 schema + GOLDEN FIXTURES ─► P1.10 parity
                                │            │                  │                │
                                │            ▼                  └──────────┐     │
                                ...
                                ├─ P4  review.py                          │     │
                                │            │                             │     │
                                └─ P5  run_prep.py (stubbed E2E)          │     │
                                             │                             │     │
   P3.2 ──────────────────────────────────┐  │        P1.9 fixtures ───────┘     │
                                          ▼  │                                   │
                            P6  template/ ◄──┴───────────────────────────────────┘
                              P6.0 GENERATE from synthetic (needs P3.2)
                              P6.1 fixture → P6.2 schema/config/firmProfile
                              → P6.3 judge/contract → P6.4 sharedConfig+baseline+judge
                              → P6.5 callJudge → P6.6 narrowing → P6.7 containment
                              → P6.8 CarverGuardrail → P6.9 guardedAgent
                              → P6.10 workflows → P6.11 evals → P6.13 report/mastra/scripts
                                             │
                            P5 ∧ P6 ─────────▼
...
**Parallel tracks — and their joins, stated honestly.** An earlier draft called track B "INDEPENDENT
of P1–P5" and called all ten P1 leaves mutually independent. **Both were false**, and rubric 13
rejects exactly that: P6.0 runs `prep/`'s generator; P1.1's cutoff test calls P1.4's function; P6.14
edits `prep/tests/test_config.py`. A parallelism claim that is wrong is worse than none — a subagent
acts on it.
...
| **B — `template/` modules** | P6.1 → P6.2 → … → P6.13 | `prep/` **implementation** | **Needs P3.2** before **P6.0** (the generator). B's *module* work can start at P0; its **generated-source gate (P6.0) waits on P3.2** |
...
| **D — drift checks** | P6.14 | — | Needs **both** `prep/`'s constants (P1.11) **and** `template/src/config.ts` (P6.2). It **writes into `prep/tests/test_config.py`** — so it is *not* a pure track-B task |
...
runs here (P6.8); its **live** assertions are excluded from `test:unit` and deferred to **R7.0**,
Phase 7's preflight. Ends green when `npm run test:unit` and `npm run typecheck` pass.

### P6.0 — **Generate the template's scenario-locked source from SYNTHETIC data** *(depends on P3.2)*
- **Spec:** §7's generation contract
- **Do:** build a synthetic `ScenarioDecision` (winner `"A"`, plausible strengths/counts) and run
  the **real** generator — `emit_template_config(synthetic_cleared_records, synthetic_decision)` —
  against P6.1's synthetic cleared set. It writes all **four** fragments mechanically:
  `config.ts`'s `DEMO_TRIGGER_RECORD_ID`, `firmProfile.ts`'s `DEMO_FIRM_PROFILE`,
  `baselineAgent.ts`'s `SCENARIO_PERSONA_INSTRUCTIONS`, and `scenario/prompts.ts` **in full**.
- **Command:** `cd prep && .venv/bin/python run_prep.py --emit-template-config --synthetic`
  (the `--synthetic` branch reads the template's fixture instead of `data/cleared/`; it makes **no**
  API calls and exists so the generator has a Phase-6 input).
- **Why this replaces the earlier draft's design.** An earlier draft hand-authored a "stand-in"
  `scenario/prompts.ts` for Phase 6 and `.skip`ped every test that depended on generated constants
  until P8.2. Both were wrong, and the second was a symptom of the first:
  - §8 states this module is **generated in full and never hand-authored**. A hand-written
    stand-in is not a scaffold, it is a **second implementation of a generated file** — and the
    thing §8's generated-only rule buys is that the eval asks the *same question the evidence was
    recorded for*. A stand-in silently reintroduces exactly the drift the rule exists to prevent,
    and it would have been tested green all through Phase 6.
  - The `.skip`s were then load-bearing: `SCENARIO_PERSONA_INSTRUCTIONS !== ""` is the check that
    the generation step **ran**, and skipping it until P8 means Phase 6 could not tell an
    un-generated persona from a generated one. Running the real generator here deletes the skip
    machinery entirely.
- **P8.2 reruns this same generator** against reviewed **real** data, and reruns the same tests.
  One generator, two inputs — synthetic then real. Nothing is hand-written at either point.
- **Dependency (stated, not glossed):** this task needs **P3.2**'s `emit_template_config`. It is
  the join that makes track B *not* wholly independent of the prep track — see §2's graph.
...
- **No skips.** These cases pass in Phase 6 because **P6.0 ran the real generator** against
...
### P6.3 — `judge/contract.ts` **(leaf — zod only)**
...
### P6.4 — `agents/sharedConfig.ts` + `baselineAgent` + `judgeAgent`
...
- **NOT `guardedAgent`** — it imports `CarverGuardrail`, which does not exist until P6.7. See the
  ordering note below.

### P6.5 — `judge/callJudge.ts`
...
- **Depends on:** `judge/contract.ts` **and** `agents/judgeAgent.ts` — **both must already exist**
  (P6.3, P6.4). An earlier draft built this before `judgeAgent`, which is a module importing a
...
### P6.6 — `tools/narrowObligations.ts`
...
- **Tests first:** `narrowObligations.test.ts` — zero-required-match; exactly-one; >5 (ranking
  ...
  **`test_narrowing_golden_parity`**; **`test_demo_trigger_record_survives_narrowing`** — no skip:
  P6.0 generated `DEMO_TRIGGER_RECORD_ID`/`DEMO_FIRM_PROFILE` from the synthetic set, and the
...
### P6.7 — `processors/tripwireContainment.ts` **(goal #8's KNOWN RISK — resolved HERE, early)**
...
### P6.8 — `processors/carverGuardrail.ts`
...
### P6.9 — `agents/guardedAgent.ts` **(only now — it imports `CarverGuardrail`)**
- **Spec:** §8 | **Creates:** `guardedAgent` — `...SHARED_AGENT_CONFIG` (**the same object**
  ...
  `maxProcessorRetries`.**
- **Depends on:** `agents/sharedConfig.ts` (P6.4) **and** `processors/carverGuardrail.ts` (P6.8).
  This is why it is not in P6.4 with the other two agents: an earlier draft built all three there,
...
### P6.10 — `workflows/compareWorkflow.ts`
...
### P6.11 — `evals/deliveryWorkflow.ts` + `evals/scorers.ts`
...
### P6.12 — `prompts.test.ts` — fair-test discipline, template-side
...
- **Why this is its own task and not a footnote:** §3's MUST-NOT list and
  ...
  it binds neither.
- **Sequencing:** `scenario/prompts.ts` is **generated by P6.0** from synthetic data, so this
...
### P6.13 — `report/`, `mastra.ts`, `scripts/`
...
### P6.14 — Cross-language drift checks (now that `config.ts` exists)
...
- **Preconditions:** P6.14's zero-cost gate green (`npm run test:unit` + `typecheck`).
...
- **Also update:** nothing to unskip — **P6.0 already ran this generator** against synthetic data,
...
- **Verify:** `cd template && npm run test:unit` — green, nothing skipped, and
  `git diff template/src/scenario/prompts.ts` shows the regenerated file differs from P6.0's only
...
| Tripwire in `.parallel()` | **P6.6** (stubs, free) + **R7.0** (live preflight) | `tripwireContainment.test.ts::test_both_tripwire_forms_normalize_identically`; `comparisonWorkflow.test.ts` asserts `status === "success"`, never `"tripwire"` |
