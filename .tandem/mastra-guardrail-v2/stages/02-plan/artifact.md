...
**Refinement 2 (round 9) fixes one bug and one word. Nothing else changed.**

| # | Item | Fix |
|---|---|---|
| **Y1** | **`user_instruction: ""` passed the gate.** The orchestrator extracted P8.0's command verbatim and executed it: the six cases the plan claimed to have tested all passed — **including the digest binding** — but an untested branch was broken. `field()`'s regex captured the two literal **quote characters**, so an empty `""` returned a truthy 2-character string, `if not (field(...) or '')` never fired, and the gate **exited 0** on an artifact carrying no human answer | `field()` now strips surrounding quotes and whitespace (via `\x22`/`\x27` hex escapes, so no quote literal has to survive the shell's own quoting) and the check is a plain `if not field('user_instruction')`. **Executed against 15 cases** — the 6 originals (which must not regress, and did not) plus `""`, `''`, whitespace-only, quote-only, bare, quoted-real, unquoted-real. The claim of empirical testing now carries the exhaustive list it describes |
| **Y2** | P8.0's quoting note called a would-be `\$` mis-escape a **"rubber stamp that always exits 3"** — backwards. A gate that always exits 3 blocks everything: a **false alarm**, fail-closed | Relabelled, and the analysis corrected while I was there: it would strand only the **thin-yield** path (healthy runs exit at `n >= 20` **before** any field is read), so "blocks everything" would have been a second wrong claim in the sentence fixing the first |

**Y1 is the sharpest lesson this plan has produced, so it is recorded rather than quietly patched.**
The field that broke was the **only** one that carries a human. `survivor_count` and `set_digest`
can both be *computed* by whatever executes this plan; `user_instruction` cannot. So the gate whose
entire argument is *mechanism over assertion* was — in the one field where a mechanism actually
matters — an assertion. And it survived a round of my own testing because I wrote the fixture list
from **the cases I had thought of**, then wrote *"executed against fixtures, not reasoned about"*
over the result. Six true rows made the list read as coverage. **A partial fixture list under a
claim of completeness is worse than no claim**: it converts an untested branch into a tested-looking
one, and it is how a checker's attention gets spent elsewhere. The rule this earns: when a task's
argument is "this is mechanical, not asserted", **enumerate the branches and test each one** —
including the ones that only fail on input a careless human would produce.

**Round 8 closes one, and it is round 7's fix applied too narrowly.**

| # | Round-7 issue | Fix |
|---|---|---|
| 1 | **P6.12b's re-derived import row was still not the scorer module's symbol map.** Round 7 corrected the judge-contract edge and stopped there, leaving three errors in the same row: `DeliveryInput` named as an import from P6.12a when **spec:5081 defines it locally** inside `evals/scorers.ts` (`z.infer<typeof DeliveryInputSchema>`) and P6.12a exports no such type; `FirmProfileSchema` named when the scorer code calls **`firmProfileForRecord(record)`** (5280/5293) and never parses a profile; and the `runEvals` **targets themselves** — `deliveryWorkflow`, `stageBWorkflow` — omitted entirely | The **whole row re-derived from §12's scorer implementation**, as a table of *from → task → exact symbols → the spec line that forces each*. Locals are now listed as locals (`DeliveryInput`, `StageBInput`, `StageBResponse`) so no one imports them. The direct `cleared-set.json` JSON import (`resolveJsonModule`, spec:3953) is named, since **no module exports `vendoredClearedSet`** |

**Why round 7 got this wrong is worth stating.** The checker's round-6 note pointed at one bad edge
(the judge contract), and I fixed **that edge** rather than re-deriving **the row**. The plan's rule
is symbol-level dependency accuracy for *every* claim in the row, not for the claim someone
happened to challenge. Fixing only what was reported is how the remaining three survived a round
that was explicitly about that row — the same shape as the renumbering defects: the site named in
the feedback gets fixed, its neighbours do not.

**A fourth unowned symbol fell out of the re-derivation.** `StageBResponse` is used at spec:5147,
5153 and 5428 and is **exported by nothing** — `schema.ts` exports `StageBResponseSchema`, not the
inferred type. Unlike the others this one is unambiguous rather than dangerous: the spec declares
its exact sibling (`type DeliveryInput = z.infer<typeof DeliveryInputSchema>`) one line above at
5081, so the local `z.infer` alias is the only reading, and it is recorded in P6.12b's locals list
rather than raised as a callout.

**Fixing round 6's issue 2 surfaced a third unowned symbol, and this one is dangerous.** Re-deriving the row
...
- **Spec:** §12, §4
- **Depends on — the module's exact import map, derived symbol-by-symbol from the spec's scorer
  implementation** (§12's code, not §8's module row alone). Every row cites the spec line that
  forces it, so a subagent can build this without searching for or inventing exports:

  | From | Task | Symbols it imports | Forced by |
  |---|---|---|---|
  | `evals/deliveryWorkflow.ts` | **P6.12a** | `deliveryWorkflow`, `stageBWorkflow` (**values** — the `runEvals` targets), `DeliveryInputSchema` (value), `type DeliveryResult` | 5273, 5384 (`target: deliveryWorkflow`); 5426 (`target: stageBWorkflow`); 5081; 5093 |
  | `firmProfile.ts` | **P6.3** | `firmProfileForRecord`, `DEMO_FIRM_PROFILE` | 5280, 5293; 5387 |
  | `schema.ts` | **P6.3** | `type ClearedRecord`, `predictsStageAViolation`, `StageBResponseSchema` | 5087 (`recordFor`'s return type); `partitionForGuardedEval`'s Stage-A filter; 5147 |
  | `judge/callJudge.ts` | **P6.6** | `runJudge` | 5108 |
  | `judge/contract.ts` | **P6.4** | **`type JudgeResult`** and `asJudgeObligation` — **nothing else** | 4195; 5108 (see the callouts below) |
  | `tools/narrowObligations.ts` | **P6.7** | `narrowObligationsPure` | 5293 |
  | `scenario/prompts.ts` | **P6.2** (generated) | `buildStageAPrompt`, `buildStageBPrompt`, `NEGATIVE_CONTROL_PROMPTS` | 5276; 5427; 5385 — **spec-issue callout below** |
  | `src/data/cleared-set.json` | **P6.1** → **P8.1** | the vendored array, **imported directly** as JSON (`resolveJsonModule: true`, spec:3953), bound to `vendoredClearedSet` — **no module exports it** | 5088 |
  | `@mastra/core/evals` | — | `createScorer`, `runEvals` | 5093; 5272, 5383 |
  | `@mastra/core` | — | `RequestContext` (**an instance**, never a `{firmProfile}` literal) | 5280, 5387 |
  | `zod` | — | `z` | 5081 |

- **Declared LOCALLY in this module — do not import these, they exist nowhere else:**
  `type DeliveryInput = z.infer<typeof DeliveryInputSchema>` — **spec:5081 defines it *inside* this
  module. An earlier draft of this task listed `DeliveryInput` as an import from P6.12a, whose
  public surface does not export it**; the import is the *Schema*, and the type is inferred here.
  `type StageBInput = { prompt: string }` (5082). `type StageBResponse` — used at 5147/5153/5428
  and **exported by nothing**; declare it exactly as the spec declares its sibling one line above:
  `z.infer<typeof StageBResponseSchema>` (which is why `StageBResponseSchema` — not a
  `StageBResponse` type — is the `schema.ts` import). Also local: `recordFor`, `extractScores`,
  `LedgerRow`, `DeliveryScorer`, the five scorers, the three §4 score ports,
  `partitionForGuardedEval`, `stageBRecords`, `runArm`, `runNegativeControl`, `runStageBEval`,
  `runScoreboard`.
- **`firmProfileForRecord`, not `FirmProfileSchema`.** The scorer code calls
  `firmProfileForRecord(record)` to build each item's `RequestContext` (5280) and again inside
  `onItemComplete` for the ledger's `candidateCount` (5293). It never *parses* a profile — that is
  `compareWorkflow`'s `requestContextSchema` job (P6.11), not a scorer's. An earlier draft named
  the schema here only because the module was related: precisely the "do not name a symbol because
  its module is nearby" failure this plan's own symbol-level rule exists to stop.
...
- **Verify — all four, and the second is not optional:**
  ...
  4. **The self-containment gate — it tests IMPORTS, not free text (orchestrator D30.1).** Two
     commands, both must exit **0**. Run from the repo root unless stated.
     - **4a — the property, by module-graph resolution.** Every file TypeScript actually resolves
       for the whole program (`src` + `scripts` + `tests`) must live inside `template/`:
       ```
       cd template && ! npx tsc --noEmit --listFiles | grep -v "/node_modules/" | grep -v "^$(pwd -P)/"
       ```
       This is success criterion 9 stated exactly: *no module in `template/` resolves anything
       outside `template/`*. It is **stricter than any literal list** — it catches a stray
       `../../carver-dags/x` or `../data/y` that no hand-written pattern anticipated — and when it
       fails it **names the offending file**. Prints nothing when green.
     - **4b — the named sins, as a fast literal backstop.** Catches an import clause that does not
       resolve at all (so 4a never sees it) and names it plainly:
       ```
       ! grep -rnE "(\bfrom|\bimport|\brequire)[[:space:]]*\(?[[:space:]]*['\"][^'\"]*(carver-showcase|\.\./prep|mastra_prep)" template/src template/tests template/scripts
       ```
     - **`template/scripts` is in scope**, though the superseded gate omitted it: `scripts/demo.ts`
       ships and runs (`npm run demo`), so it is a module in `template/` and criterion 9 reaches it.
  - **🔴 WHY THIS REPLACED A SUBSTRING GREP — do not revert it (D30.1).** The superseded gate was
    `grep -rn "carver-showcase\|\.\./prep\|mastra_prep" template/src template/tests` → no hits. It
    is **RED on correct code**: it has 7 hits today and **every one is a prose doc comment**
    (`config.ts`, `schema.ts`, `narrowObligations.ts`, `baselineAgent.ts`, `judge/contract.ts`,
    `scenario/prompts.ts`) explaining which Python function each TS port must stay in lockstep with.
    **Not one is an import.** So the obvious way to green it is to **delete exactly the comments
    that document the cross-language contract** — a gate that makes the code worse while reporting
    the goal met. Criterion 9 is about **dependencies**: a comment naming a Python file is not a
    dependency, it is documentation of one that deliberately does not exist. **The doc comments
    stay.**
  - **Proven able to fail** (not merely reasoned about): adding a real, resolvable
    `import prepGolden from "../../prep/tests/fixtures/scoring_golden.json"` to `tests/fixtures.ts`
    — the realistic violation here, since the template keeps its **own** byte-identical copy of that
    fixture — turns **both** commands red (4a names the resolved `prep/.../scoring_golden.json`; 4b
    names the clause), and removing it returns both to green with all 7 doc comments untouched.
...
- **ONE command, both paths. There is no second variant to invent.** An earlier draft stopped at
  ...
  def field(name):
      ...
      if not m:
          return None
      # Strip surrounding quote characters (\x22 = double, \x27 = single) and whitespace.
      # Written as hex escapes so no quote literal has to survive the shell's own quoting.
      # WITHOUT this, user_instruction: \"\" captures the two quote CHARACTERS -> a truthy
      # 2-character string -> the emptiness check below never fires. See the note after.
      return re.sub(r'^[\x22\x27\s]+|[\x22\x27\s]+\$', '', m.group(1))
  ...
  if not field('user_instruction'):
      problems.append('user_instruction must quote the user verbatim (non-empty)')
  if problems:
...
- **This command was executed against fixtures — all fifteen cases below — before being written
  down.** Not reasoned about:

  | # | Fixture | Exit |
  |---|---|---|
  | A | 14 records, no artifact | **3** |
  | B | artifact with a stale `survivor_count: 99` | **3** |
  | C | correct artifact | **0** |
  | D | the 14 records replaced by a **different** 14 (same count) | **3** — on `set_digest` |
  | E | 25 records, no artifact | **0** |
  | F | boundary: 20 / 19 | **0** / **3** |
  | G | `decision: proceed` (lowercase) | **3** |
  | H | `user_instruction: ""` | **3** |
  | I | `user_instruction: ''` | **3** |
  | J | `user_instruction: "   "` (whitespace only) | **3** |
  | K | `user_instruction: "` (quote only) | **3** |
  | L | `user_instruction:` (bare, no value) | **3** |
  | M | a real quoted instruction / a real unquoted one | **0** / **0** |

- **Cases H–M exist because the first version of this gate passed case H, and the claim right above
  this one did not cover it.** Six cases were executed and stated; the emptiness branch was not one
  of them, and it was broken: `field()`'s old regex captured the two literal **quote characters**,
  so `user_instruction: ""` returned a **truthy** 2-character string and the check never fired.
  **That was the one field that mattered.** `survivor_count` and `set_digest` can both be *computed*
  by whatever is executing this plan; `user_instruction` is the only field that can come **only**
  from a human having actually answered. The gate whose entire argument is *mechanism over
  assertion* was, in the field carrying the human, an assertion. A partial fixture list under a
  claim of empirical testing is worse than no claim: it reads as coverage. The list is now
  exhaustive over every branch, and D and F are re-run after every change to this command —
  the count and digest bindings must not regress while the quoting is fixed.
- The `\$` in the regex is escaped for the shell's double quotes and reaches Python as an
  end-of-line anchor — verified. Mis-escaped, every field would read `None`, every problem would be
  appended, and **every thin-yield run would exit 3 no matter what the artifact said** — the
  acknowledgment path would be permanently unusable. (Healthy runs are unaffected: `n >= 20` exits
  **before** any field is read.) That is a **false alarm** — fail-closed, loud, and impossible to
  ship silently, since the first real escalation would hit it. It is the *safe* direction to fail,
  and it is emphatically **not** a rubber stamp: an earlier draft of this note called it one, which
  had the direction exactly backwards. A gate erring toward "blocked" is the failure this project
  can afford; a gate erring toward "proceed" is the one it cannot.
...
- **Also update — `template/README.md`'s "Status — what is built, and what is not" section
  (P6.16). NOT optional; this task is the moment its central claim becomes false.** P6.16 wrote
  that section because at Phase 6 the vendored file is **P6.1's synthetic 6-record fixture with
  non-resolving illustrative URLs**, and a README claiming real human-reviewed records with
  resolvable citations would have been a **false claim shipped to the partner** — the one artifact
  where a wrong claim reaches Mastra directly. **This task inverts that**: once the real set lands,
  the synthetic-fixture disclaimer is itself the false statement, and it slanders our own data.
  Rewrite the bullets to the post-vendoring truth and state the **real** record count (goal #11:
  report the true number, whatever it is — a small set of proven failures is the success case).
  The section is deliberately written as one contiguous, clearly-headed block so this edit is
  localized. **Leave the third bullet** (live scoreboard bars not yet wired into `npm test`) —
  it stays true until Phase 9 wires them; **P9's criterion-6 task owns that one.**
