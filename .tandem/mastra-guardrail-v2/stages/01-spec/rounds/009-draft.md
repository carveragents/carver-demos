...
**Stage:** 01-spec **Round:** 9 (refinement cycle 2 — revision of the round-8 approved artifact)
**Status:** Draft
...
| `mastra.ts` | `mastra: Mastra` — registers **all three agents and all three workflows** (§8/§12; the two eval workflows MUST be registered or their steps have no `mastra` and `npm test` cannot run). `new Mastra({ agents: { baselineAgent, guardedAgent, judgeAgent }, workflows: { compareWorkflow } })` | `@mastra/core`, `agents/*`, `workflows/compareWorkflow.ts` | none (construction only) |
...
`{ blocked: true, delivered_text: null, violated_obligation_ids }`. So §10's live containment
test really does exercise the code the scoreboard depends on — the claim is now true of the
part that was ever at risk, and no longer asserted of the part that never could be. The
baseline arm passes through untouched (no processor, so never `tripped`) and comes out as
`{ blocked: false, delivered_text }`.
