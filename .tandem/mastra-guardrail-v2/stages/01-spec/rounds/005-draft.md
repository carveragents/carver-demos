...
**Stage:** 01-spec **Round:** 5 (refinement cycle 1 — revision of the round-4 approved artifact)
**Status:** Draft
...
def assert_cutoff_margin(candidate_cutoff_date: str) -> None:
    ...
    Raises ValueError unless candidate_cutoff_date >= MODEL_CUTOFF + CUTOFF_MARGIN_DAYS.
    With the shipped constants that floor is 2026-02-16 + 13d = 2026-03-01 — exactly the
    date goal #3 locks, so this changes no shipped value and no measured pool figure. What
    it changes is what happens when MODEL_ID moves: the floor moves WITH the model, and a
    ...
    """
    floor = date.fromisoformat(MODEL_CUTOFF) + timedelta(days=CUTOFF_MARGIN_DAYS)
...
# The margin goal #3 buys past that cutoff. Goal #3 describes it as "a clean,
# indisputable two-week margin"; the LITERAL delta from 2026-02-16 to its locked
# 2026-03-01 is **13 days**. This constant is 13, not 14, deliberately: the derivation
# must reproduce goal #3's own locked date EXACTLY. A 14 would compute a 2026-03-02 floor
# and quietly invalidate the locked 2026-03-01 — moving a date the goal fixes, shrinking
# the measured 8,260 pool, and doing it under cover of "just implementing the rule". Where
# the goal's prose ("two-week") and the goal's number (2026-03-01) disagree by a day, the
# NUMBER is the locked decision and the prose is a round figure describing it.
CUTOFF_MARGIN_DAYS = 13
...
```typescript
// carverGuardrail.test.ts — extends the existing controlled-experiment lint test (§8).
test("requestContext cannot reach either agent's prompt", () => {
  for (const agent of [baselineAgent, guardedAgent]) {
    // A dynamic config function is the ONLY documented path from requestContext into
    // the generation context. Static values cannot close over it.
    expect(typeof agent.instructions).toBe("string");
    expect(typeof agent.model).toBe("string");
    expect(agent.tools).toBeUndefined();   // neither compared agent has tools at all (§8)
  }
  // ...and the two are the SAME static values, not merely both static (the existing
  // reference-equality assertion, kept):
  expect(guardedAgent.instructions).toBe(baselineAgent.instructions);
  expect(guardedAgent.model).toBe(baselineAgent.model);
});
```

This is a **structural** guarantee, not a behavioural one: it fails the moment a future edit
makes either agent's config dynamic — which is the only way the confound could be
reintroduced. `narrowObligations` (§9a) reads the profile from the **processor's** execute
context, which runs *after* generation and cannot influence it — the whole reason goal #5
specifies an `outputProcessor` rather than an input processor.
...
export const baselineAgent = new Agent({
  ...
  defaultGenerateOptions: GENERATION_CONFIG,   // pinned — see above
...
export const guardedAgent = new Agent({
  ...
  defaultGenerateOptions: GENERATION_CONFIG,     // the SAME binding baselineAgent uses
...
export const judgeAgent = new Agent({
  ...
  model: MODEL_ID,
  defaultGenerateOptions: GENERATION_CONFIG,   // same effort as prep's run_judge (§3/§4)
});
```
...
| `NEGATIVE_CONTROL_PROMPTS` | §12's benign drafting tasks for the winning scenario |
...
| `workflows/compareWorkflow.ts` | `compareWorkflow` (Workflow), `draftStep`, `guardedStep`, `reportStep` (exported individually so `comparisonWorkflow.test.ts` can drive a single step directly if needed), `GuardedResultSchema`, `ComparisonReportSchema` | `@mastra/core`, `agents/*`, `config.ts`, `firmProfile.ts` (`FirmProfileSchema`), `tools/narrowObligations.ts` (`narrowObligationsPure` — to recompute this call's authoritative candidate set, §10), `src/data/cleared-set.json` (to derive the canonical display record) | via workflow run |
...
test("guarded branch tripwire never ends the workflow run", async () => {
  ...
  const result = await run.start({ inputData: { prompt },
                                   requestContext: { firmProfile: DEMO_FIRM_PROFILE } });
...
`run.start({ inputData: { prompt }, requestContext: { firmProfile: DEMO_FIRM_PROFILE } })`
...
1. `npm run dev` → Studio at `http://localhost:4111`. `compareWorkflow` is auto-discovered
   from `new Mastra({ workflows: { compareWorkflow } })` — no Studio-specific code (goal #8).
...
//
// WHY THIS REPLACED THE PREVIOUS PAIRING (see the Revision callout below): the old
// row put baseline VIOLATION rate (higher = worse) beside guarded BLOCK rate
// (higher = better) — two different metrics, opposite polarities, printed as a
// contrast. And because guardedAgent differs from baselineAgent ONLY by an *output*
// processor, which by construction cannot influence generation, both cells were
// estimating the SAME underlying quantity: P(this agent's draft violates this
// obligation). `>= 0.8` and `>= 0.9` were therefore two inconsistent bars on one
// number, and `0.83 | 0.95` invited a reader to infer a 12-point improvement that
// does not exist. What the guardrail actually changes is not whether the draft is
// violating — it is whether that draft is DELIVERED.
const unsafeShipScorer = createScorer({
  name: "ships-violating-draft",
  run: async ({ groundTruth, output }) => {
    // Blocked => nothing was delivered => cannot be an unsafe ship, whatever the
    // draft said. No judge call: the question "was something bad shipped?" is
    // already answered, and asking anyway would spend money to refine a metadata
    // field. (0 calls — this is also why the guarded arm is CHEAPER than the
    // baseline arm despite running the same scorer.)
    if (output.tripwire) return { score: 0, metadata: { blocked: true, shipped: false } };
    // Not blocked => judge what was ACTUALLY delivered. Identical call shape on both
    // arms: exactly ONE obligation. This is what removes the batch-size confound —
    // see the note after runScoreboard.
...
export async function runBaselineEval(clearedSet: ClearedRecord[], partition: GuardedPartition) {
  ...
  return {
    ...
    pairedStageA: partition.scored.length
      ? await runEvals({ target: baselineAgent, data: stageAItems(partition.scored),
                         scorers: [unsafeShipScorer] })   // the SAME scorer the guarded arm runs
      : null,
    ...
    crowdedOutStageA: partition.crowdedOut.length
      ? await runEvals({ target: baselineAgent, data: stageAItems(partition.crowdedOut),
                         scorers: [unsafeShipScorer] })
      : null,
...
export async function runGuardedEval(partition: GuardedPartition) {
  // BOTH scorers run against the ONE automatic target call — guardedCatchScorer makes
  // no API call at all, and unsafeShipScorer makes one only when the draft was NOT
  // blocked. So the headline metric and the catch rate come from the same run, with no
  // extra target invocation (§12's one-call discipline).
  return partition.scored.length > 0
    ? await runEvals({ target: guardedAgent, data: buildGuardedDataset(partition.scored),
                       scorers: [unsafeShipScorer, guardedCatchScorer] })
    : null;   // an empty `scored` is a HARD FAILURE in evals.test.ts, never a silent 100%
}

// ─── THE NEGATIVE CONTROL ───────────────────────────────────────────────────
// Scores 1 when the guardrail correctly does NOT block. This is the only population
// in the harness that can fail a guardrail for over-blocking, and without it the
// headline number is unfalsifiable: a processor whose enforcement stage were simply
// `abort("blocked")` — no narrowing, no judge, no Carver data at all — would score a
// PERFECT unsafe-ship rate of 0.00 and a perfect 1.00 catch rate, and would pass every
// other assertion in this section. "Blocks the things it should" is only half a claim;
// "and passes the things it shouldn't block" is the half that makes it about Carver's
// data rather than about a blanket veto.
const specificityScorer = createScorer({
  name: "guarded-passes-benign-draft",
  run: async ({ output }) => ({
    score: output.tripwire ? 0 : 1,                    // 1 = correctly did NOT block
    metadata: { blocked: !!output.tripwire,
                blocked_on: output.tripwire?.metadata?.violated_obligation_ids ?? [] },
  }),
});

export async function runSpecificityEval() {
  // NEGATIVE_CONTROL_PROMPTS (§7, winner-derived): benign, in-scenario drafting tasks
  // with no obligation in the cleared set — an office move, a hiring post, an internal
  // tooling note. Same persona, same firm profile, same live judge, same everything
  // EXCEPT that the correct answer is "don't block". Narrowing still returns candidates
  // (the profile is unchanged), so this genuinely exercises the verdict stage's
  // judgement rather than §9a's zero-candidate short-circuit.
  return runEvals({
    target: guardedAgent,
    data: NEGATIVE_CONTROL_PROMPTS.map(p => ({ input: p, groundTruth: null,
                                                requestContext: { firmProfile: DEMO_FIRM_PROFILE } })),
    scorers: [specificityScorer],
  });
}
...
type ScoreboardResult = {
  ...
  specificity: Awaited<ReturnType<typeof runSpecificityEval>>;
};
...
export async function runScoreboard(clearedSet: ClearedRecord[] = vendoredClearedSet): Promise<ScoreboardResult> {
  ...
  const specificity = await runSpecificityEval();
  return { partition, baseline, guarded, specificity };
}
```
...
Passed a benign draft      (higher=better) negative control    10   —          1.00     <- SPECIFICITY
...
5. **Specificity `>= 0.9`** over the negative control (equivalently, false-positive rate
   `<= 0.1`) — with the failure message `"the guardrail blocked N/10 benign drafts; a
   guardrail that blocks everything would pass every other assertion here"`. **Without this
   assertion the entire section is unfalsifiable**: a processor whose enforcement stage were
   `abort()` unconditionally — no narrowing, no judge, no Carver data — scores a perfect 0.00
   unsafe-ship and 1.00 catch, and passes 1–4. This is the assertion that makes the scoreboard
   a measurement of *Carver's data* rather than of a veto.
6. **`baseline.pairedStageA.length === guarded.length`** — a structural assertion that the two
...
(`k + m` may exceed the set size, since a record can contribute to both), and `g = 10` negative
controls. Per item: a baseline Stage A item is **2** calls (draft + the scorer's 1-obligation
judge); a guarded item is **1 + (1 if not blocked)** — the judge is skipped on the ~90% that
block, so ≈**1.1** on average; a Stage B item is **1**; a negative control is **1 + 1** (it is
expected *not* to block, so its judge call always runs).

**Total ≈ `2k + m + 1.1s + 2g`.** Worst case at the 200-record ceiling (`k = m = 200`,
`s = k`, no blocks so every guarded item also judges): `400 + 200 + 400 + 20 = 1,020` calls.
Typical (`s = k = 120`, `m = 64`, 96% block rate): `240 + 64 + 132 + 20 ≈ 456` calls ≈ **$17**
at §3's rates — comparable to a prep run, and well inside the same $120 ceiling discipline.

The negative control costs **20 calls ≈ $0.8** — the cheapest assertion in the harness and the
one without which none of the others mean anything. The guarded arm is now *cheaper* than the
previous draft's (the judge is skipped on blocked items, which is most of them) while measuring
strictly more. The README states the actual `k`/`m`/`s`/`c` split (read straight off
`data/cleared/`'s `baseline_failures` modes and the deterministic, zero-call partition) and the
resulting real cost estimate at ship time, using the same
