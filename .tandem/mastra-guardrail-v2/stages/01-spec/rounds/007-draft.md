...
**Stage:** 01-spec **Round:** 7 (refinement cycle 1, round 3 — revision of the round-4 approved artifact)
**Status:** Draft
...
// onItemComplete's `scorerResults` is keyed by scorer id, but each VALUE is the full
// result object scorer.run(...) returned — `.score` plus run metadata and optional step
// data — not a number. An earlier draft wrote `scores: scorerResults`, which neither
// satisfies Record<string, number> nor lets mean(ledger.scores[k]) compare against
// runEvals' numeric averages: the promised ledger-versus-average assertion could not
// have held, and the subgroup rows would have averaged objects.
  ...
  for (const id of expectedIds) {
    ...
    if (typeof score !== "number" || !Number.isFinite(score)) {
      throw new Error(`scorer "${id}" produced ${JSON.stringify(scorerResults[id])} — expected a `
        + `finite numeric .score. The per-item ledger is asserted against runEvals' own averages `
...
// MastraScorer<...>[] — the pinned public scorer-entry type. `Scorer[]` (an earlier
// draft's) is not a type @mastra/core@1.51.0 exports.
async function runArm(arm: "baseline" | "guarded", records: ClearedRecord[],
                       scorers: MastraScorer<typeof DeliveryInputSchema, typeof DeliveryResultSchema>[]
  ...
  const result = await runEvals({
    ...
    onItemComplete: ({ item, targetResult, scorerResults }) => {
      // targetResult is the WORKFLOW result, so its output is DeliveryResult — the same
      // typed shape the scorers saw. blocked/violated ids are read from it directly, never
      // re-derived from scores (which would invert the dependency: the ledger exists to
      // explain the scores, not to guess at them).
      const record = item.groundTruth as ClearedRecord;
      // PARSED, not cast. `targetResult` is runEvals' generic result type, not
      // parameterized by our schemas the way a scorer's `run.output` is — so this is the
      // one boundary where the delivery shape arrives untyped. Parsing validates it
      // (a cast would assert it), and the ledger is what every printed n and subgroup
      // rate is computed from, so a malformed result must fail here rather than surface
      // as a wrong percentage.
      const out = DeliveryResultSchema.parse(targetResult.result);
...
async function runNegativeControl() {
  ...
  const result = await runEvals({
    ...
    onItemComplete: ({ targetResult, scorerResults }) => {
      const out = DeliveryResultSchema.parse(targetResult.result);   // parsed, not cast — as above
      ledger.push({ recordId: "", arm: "guarded", blocked: out.blocked,
                    ...
                    scores: extractScores(scorerResults, [benignPassScorer.id]) });   // same checked extraction
    },
  });
  return { ledger, averages: result.scores };
}
