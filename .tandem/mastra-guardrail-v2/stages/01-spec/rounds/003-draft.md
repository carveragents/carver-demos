...
**Stage:** 01-spec **Round:** 3 (revision of the previous run's round-5 artifact)
**Status:** Draft
...
| 14 | **The multi-violation invariant was documented but unenforced.** `record.id === violated_obligation_ids[0]` was a comment; nothing checked id uniqueness, and nothing checked the ids were records the template actually ships — so stale or forged metadata could pass Zod and leave audit, scoring and the report each citing a different obligation | §10 — the invariants are `superRefine`d on the union (uniqueness, display-record equality) and `buildBlockedResult` additionally validates known-record membership, which Zod structurally cannot (only the step holds the vendored set); three negative tests |
...
class Reservation:
    ...
    def settle(self, usage: dict) -> None:
        """TERMINAL — the normal path: a response arrived with usage. Replaces the held
        provider-maximum with the call's REAL cost, returning the difference to the
        budget. After this, this call contributes exactly its true bill to
        spend_so_far_usd.

        Also runs the anomaly check: actual <= expected_max_usd. That comparison is
        against the TIGHT estimate, not against amount_usd (which the provider's caps
        make unfalsifiable). A violation means our model of the request is wrong — a
        call site reserved against different text than it sent, framing outgrew the
        allowance, or OpenAI now bills for something uncounted. Each is a real bug; the
        run poisons and stops rather than continuing on a ledger that no longer
        predicts anything. The CEILING is unaffected either way (see the proof).
        """
...
class SpendBudget:
    ...
    def reserve(self, payload: dict) -> "Reservation":
        ...
        r = Reservation(amount_usd=amount, expected_max_usd=expected_max, budget=self)
        self._open.add(id(r))
        return r

    def assert_no_open_reservations(self) -> None:
        """Called by run_prep.py at the end of a run. An open reservation means some
        code path reserved and then neither settled, released, nor finalized — a bug
        that would leave spend over-stated. It is safe (over-statement never breaks the
        ceiling) but it is still wrong, and silence would hide it."""
        if self._open:
...
try:
    ...
    res.settle(response.usage)                # may raise BudgetPoisoned -> caller stops
```

Exactly one terminal operation runs on every path — `settle` on the `else`, `release` or
`finalize_unknown` on the `except`. **A retry never inherits the previous attempt's
reservation**: the first attempt terminates its own handle before raising, and the retry
reserves afresh. That is the whole of issue 1's fix, and `assert_no_open_reservations()` at
the end of `run_prep.py` catches any future code path that forgets.
...
|---|---|---|---|
| `settle(usage)` | `+= b − A` (i.e. becomes `spend_before + b`) | `b <= A` ⟹ `<= spend_before + A <= ceiling` ✓ | books `b` exactly ✓ |
| `release(...)` | `−= A` (back to `spend_before`) | strictly lower ✓ | provider confirmed `b = 0` ✓ |
| `finalize_unknown(...)` | unchanged (keeps `A`) | already `<= ceiling` ✓ | `b <= A` whatever `b` was ✓ |
...
def decide_scenario(client, trial_pool: list[dict], cfg: Settings, budget: "SpendBudget") -> ScenarioDecision:
    ...
    if not sufficient:
        return ScenarioDecision(
            outcome="insufficient_trial", winner=None, stop_reason=stop_reason, ...)
...
const guardedStep = createStep({
  ...
  execute: async ({ inputData, mastra, requestContext }) => {
    ...
    const buildBlockedResult = (reason: string, processorId: string, metadata: unknown) => {
      ...
      // Structural completeness. The three INVARIANTS beyond mere presence —
      // uniqueness, display-record equality, and known-record membership — are checked
      // separately below/by GuardedResultSchema, because Zod can enforce the first two
      // from the value alone but CANNOT check membership: that needs the vendored set,
      // which only this step has. A stale or hand-forged metadata object would
      // otherwise satisfy every type and still make the report cite an obligation the
      // template does not ship.
      const unknownIds = Array.isArray(violatedIds)
        ? violatedIds.filter(id => !vendoredClearedSet.some(r => r.id === id))
        : [];
      if (typeof blockedDraft !== "string" || !record
        ...
        throw new Error(`CarverGuardrail tripwire fired but metadata is incomplete or unsound `
          ...
          + `${unknownIds.length ? `, unknown ids: ${unknownIds.join(",")}` : ""}) `
...
| `comparisonWorkflow.test.ts` | tripwire-containment proof, asserting the discriminated union's non-null `blocked_draft`/`reason`/`processorId`, that `violated_obligation_ids` **contains** `DEMO_TRIGGER_RECORD_ID`, and that `record.id === violated_obligation_ids[0]`, on a real live run (§10); **`test_incomplete_metadata_fails_loudly`** — a stubbed tripwire whose metadata omits `violated_obligation_ids` (or supplies `[]`) makes `guardedStep` throw the named error rather than emit a result the report cannot render; **the three negative invariant cases**, each asserting a throw rather than a parsed result: `violated_obligation_ids` containing a **duplicate** id; containing an id **not present in the vendored cleared set** (stale/forged metadata — the case Zod structurally cannot catch, since only the step holds the set); and a **display mismatch** where `record.id !== violated_obligation_ids[0]` (which would make audit, scoring and report each cite a different obligation while every field still type-checks) | **Yes** (the metadata-completeness and invariant cases: **No**) |
