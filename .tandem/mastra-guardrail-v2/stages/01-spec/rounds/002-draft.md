...
**Stage:** 01-spec **Round:** 2 (revision of the previous run's round-5 artifact)
**Status:** Draft
...
| 7 | **The hard-ceiling claim was still false.** `record_actual()` runs *after* the billable call, so it could only *detect* an overspend, never prevent one — "the run cannot spend past the ceiling, full stop" did not survive the ordering | §3 — `reserve()` now gates on `_effective_ceiling_usd` (= ceiling − one call's maximum possible input cost, bounded by the provider's own `MODEL_MAX_CONTEXT_TOKENS`), making the ceiling hold **unconditionally**, with a written proof and a test driving the worst case |
...
Cap 2 closes the hole. Rather than *detecting* an overage after the fact, `reserve()` sets the
worst conceivable one aside **up front**: it gates against an `_effective_ceiling_usd` equal to
the configured ceiling **minus one call's maximum possible input cost**. Because a detected
anomaly poisons the budget permanently, **at most one** call can ever overshoot its
reservation; cap 2 bounds that one call; and the subtraction has already paid for it. The
configured `total_spend_ceiling_usd` therefore holds **unconditionally** — proof below the
code.

The byte-count and allowance terms below are still worth keeping: they make each reservation
tight enough that normal operation never trips the anomaly path at all. But **the ceiling no
longer depends on them being correct.** That is precisely the difference between this version
and the last: the guarantee now rests only on limits the provider itself enforces, and the
estimates only affect how often the run stops early.
...
# than the real gap — deliberately, since over-reserving costs nothing (record_actual
# returns the headroom immediately) while under-reserving would break the ceiling.
# It is a DECLARED ALLOWANCE, not a measurement, and record_actual() checks it on
# every call (see below) rather than trusting it.
...
def reservation_basis_tokens(payload: dict) -> int:
    """The input-token reservation basis: one reserved token per UTF-8 byte of the
    COMPLETE SDK-ready kwargs dict, PLUS a declared conservative overhead allowance.

    This is deliberately NOT claimed to equal the transmitted wire request (see the
    discussion above — the SDK serializes independently and may add framing/default
    fields; json.dumps' rendering and the wire bytes are related, not identical).
    ...
    (1) and (2) are proofs. (3) is an ALLOWANCE, and record_actual() VERIFIES it
    against the provider's own reported prompt_tokens on every single call, poisoning
    the ledger and hard-stopping the run if it is ever insufficient. The ceiling is
    therefore guarded by a bound that is checked, not one that is merely asserted.
    """
    return (estimate_tokens(json.dumps(payload, ensure_ascii=False))
            + REQUEST_OVERHEAD_ALLOWANCE_TOKENS)

class BudgetExhausted(Exception):
    """The single exception type meaning 'stop the run'. reserve() is the only raiser;
    its docstring states the exact conditions (never duplicated here)."""
...
class SpendBudget:
    def __init__(self, ceiling_usd: float, price_in: float, price_out: float):
        ...
        # Set ONE call's maximum conceivable input cost aside up front. This is what
        # makes ceiling_usd a real pre-call ceiling instead of a post-call observation:
        # at most one call can ever overshoot its reservation (record_actual poisons the
        # budget on the first one), and MODEL_MAX_CONTEXT_TOKENS is the provider's own
        # hard cap on that call's billable input. See the proof below.
        self._max_single_call_overage_usd = MODEL_MAX_CONTEXT_TOKENS * price_in / 1_000_000
        self._effective_ceiling_usd = ceiling_usd - self._max_single_call_overage_usd
        if self._effective_ceiling_usd <= 0:
            raise ValueError(
                f"ceiling_usd=${ceiling_usd} does not exceed one call's maximum possible input "
                f"cost (${self._max_single_call_overage_usd:.2f} = MODEL_MAX_CONTEXT_TOKENS x "
                f"price_in) — no call could ever be reserved, so the budget is unusable")

    def reserve(self, payload: dict) -> float:
        """Computes this call's WORST-CASE cost from (a) reservation_basis_tokens()
        over the COMPLETE SDK-ready payload (build_request_payload()'s output — never
        just its text fields, never a "typical" guess) and (b)
        payload["max_completion_tokens"] (an API-ENFORCED hard cap — OpenAI cannot
        return MORE completion tokens than requested, so this term is exact, not
        estimated).

        Gates on _effective_ceiling_usd (= ceiling_usd - one call's maximum possible
        input cost), NOT on ceiling_usd directly. That subtraction is what makes
        ceiling_usd a genuine pre-call ceiling rather than a post-hoc observation: it
        pre-pays for the single worst overage the provider could ever bill, and
        record_actual()'s poisoning guarantees there can never be a second. See the
        proof below.
        ...
        RAISES BudgetExhausted in exactly two cases, and no others:
          ...
          2. spend_so_far_usd + worst_case > _effective_ceiling_usd.
        Returns the reservation amount (float) on success; the caller MUST pass it to
        record_actual() once the real response is in hand. Called before EVERY API
        call, including retries, which reserve independently, each with its own
        ...
        if self.spend_so_far_usd + worst_case > self._effective_ceiling_usd:
            raise BudgetExhausted(f"reserving ${worst_case:.4f} would exceed the effective "
                                   f"${self._effective_ceiling_usd:.2f} ceiling "
                                   f"(${self.ceiling_usd:.2f} configured, less "
                                   f"${self._max_single_call_overage_usd:.2f} held back for one "
                                   f"call's maximum possible input; "
    ...
    def record_actual(self, reservation: float, usage: dict) -> None:
        ...
        ASSERTS actual <= reservation. This assertion does NOT enforce the ceiling —
        it cannot, since it runs after the call is already billed; ceiling_usd is
        enforced by reserve()'s _effective_ceiling_usd gate (above). What this
        assertion does is make the overage a ONE-TIME event: it is the trigger for
        poisoning, and the proof below depends on at most one call ever overshooting.
        A violation means one of the reservation's assumptions has genuinely stopped
        holding — a code path reserved against the wrong text, framing larger than
        REQUEST_OVERHEAD_ALLOWANCE_TOKENS, or OpenAI billing for something not counted
        here. Each is a real reason to stop; none is a reason to continue on a ledger
        that is no longer tight. If it IS violated: this method does NOT silently
...
**Proof: `spend_so_far_usd <= ceiling_usd` holds at every point in the run, unconditionally.**
Write `E = _effective_ceiling_usd = ceiling_usd - M`, where `M = MODEL_MAX_CONTEXT_TOKENS x
price_in / 1e6` is one call's maximum possible input cost.

*Invariant:* after every completed call, `spend_so_far_usd` = the sum of that run's **actual**
billed costs (`reserve()` adds `worst_case`; `record_actual()` adds `actual - worst_case`;
they telescope).

*Every issued call was gated.* `reserve()` returns normally only when `spend_so_far + worst_case
<= E`. So immediately before any call is issued, `spend_so_far + worst_case <= E`.

*Case 1 — the call behaves (`actual <= worst_case`), which is every call in a healthy run.*
Then `spend_so_far_after = spend_so_far + actual <= spend_so_far + worst_case <= E < ceiling_usd`. ∎

*Case 2 — the call overshoots (`actual > worst_case`).* Decompose `actual = prompt_tokens x
price_in + completion_tokens x price_out`. The provider caps each term independently:
`completion_tokens <= max_completion_tokens` (cap 1, so the output term never exceeds what was
reserved) and `prompt_tokens <= MODEL_MAX_CONTEXT_TOKENS` (cap 2 — a larger request is
*rejected*, never billed). Hence the input term is `<= M`, and
`actual <= M + (reserved output term) <= M + worst_case`. Therefore
`spend_so_far_after = spend_so_far + actual <= spend_so_far + worst_case + M <= E + M = ceiling_usd`. ∎

*Case 2 can happen at most once.* `record_actual()` sets `_poisoned = True` on the first
overshoot, and `reserve()` raises unconditionally thereafter — so no further call is ever
issued, and `spend_so_far_usd` never moves again. There is no second overage to accumulate.

**Both caps are the provider's, not ours.** Nothing in this proof depends on
`REQUEST_OVERHEAD_ALLOWANCE_TOKENS` being large enough, on `json.dumps` resembling the wire
format, or on the byte-per-token argument. Those only determine whether Case 2 ever fires
(it should not) and how tightly the run packs into its budget — not whether the ceiling holds.
This is the **hard dollar ceiling rubric 12 requires**, and it is now literally true rather
than true-in-practice: `total_spend_ceiling_usd` is never exceeded, for any prompt, any
provider framing change, and any bug in our own estimates.

`test_budget.py::test_ceiling_never_exceeded_even_when_reservation_is_wrong` proves Case 2
directly: it drives a `SpendBudget` to just under `E`, then feeds `record_actual` a `usage`
reporting `prompt_tokens == MODEL_MAX_CONTEXT_TOKENS` (the provider's own maximum, i.e. the
worst overage physically possible), and asserts `spend_so_far_usd <= ceiling_usd` still holds,
that `BudgetPoisoned` was raised, and that the next `reserve()` raises `BudgetExhausted`.
...
`test_budget.py::test_record_actual_normal_case` constructs a small-ceiling budget, reserves
against a real payload built via `build_request_payload`, calls `record_actual` with a `usage`
dict at the honest worst case (`completion_tokens == max_completion_tokens`, `prompt_tokens ==
reservation_basis_tokens(payload)`), and asserts `spend_so_far_usd <= ceiling_usd` with no
exception raised — the normal path, where `actual <= reservation` holds.
`test_ceiling_never_exceeded_even_when_reservation_is_wrong` covers the path that actually
matters (the proof's Case 2): it feeds `record_actual` a `usage` reporting `prompt_tokens ==
MODEL_MAX_CONTEXT_TOKENS` — the largest input the provider could ever bill, i.e. the worst
reservation failure physically possible rather than a token overshoot — and asserts that
`spend_so_far_usd <= ceiling_usd` **still holds**, that `BudgetPoisoned` is raised, that
`spend_so_far_usd` reflects the true (higher) cost rather than a comfortable fiction, and that
a subsequent `reserve()` raises `BudgetExhausted`. That is the "terminate further calls on any
accounting anomaly" contract *and* the hard-ceiling proof, in one test.
`test_spend_budget_rejects_price_below_pinned_floor` constructs
`SpendBudget(ceiling_usd=100, price_in=0.001, price_out=0.001)` and asserts `__init__` raises
`ValueError` — proving the floor guard fires independent of `load_settings()`'s own equivalent
check (§13).
...
> summed *reservations*, which double-counts: `record_actual()` returns each reservation's
> headroom immediately, so `spend_so_far` tracks **actual** spend and the ceiling check is
> `actual_so_far + this_one_call's_reservation <= ceiling`, never a sum of reservations.
...
> (worst case ≈ 74% of ceiling) while staying ~7x the typical run. **The hard-ceiling
...
**The ceiling check's actual peak.** Because `record_actual()` trues up immediately, the
binding quantity is `actual_so_far + the single largest reservation`. The largest reservation
is a Judge call carrying a maximum-length draft: basis ≈ 14,500 payload bytes + 1,024
allowance = 15,524 input tokens, so `(15,524 × $5 + 1,200 × $30) / 1e6 ≈ $0.114`. Peak ≈
`$88.4 + $0.11 ≈ **$88.5**` — against the **effective** ceiling of `$120 − $5 = $115` (the
$5 being `MODEL_MAX_CONTEXT_TOKENS × $5/1M`, held back by `SpendBudget.__init__` to make the
proof above work), that is **77%**, so even the pathological case completes both the trial and
the full sweep with real margin.
...
- The **guarantee** is unconditional and independent of every estimate on this page:
  ...
  `max_completion_tokens` and context-window caps. It holds for any ceiling value, any prompt
  size, any framing change, and any error in our token arithmetic. This is what rubric 12
  requires, and it is the only claim here that is load-bearing for safety.
- The **sizing** — is $120 *comfortable*? — rests on the token estimates above, which are
  ...
  specified: the run stops at `_effective_ceiling_usd` with `stop_reason="spend_ceiling"`,
  reports how many records it probed and how many survived, and ships the smaller set. That is
  goal #11's discipline applied to spend — report the true number, never pad, and never quietly
  overspend either. A genuine runaway (e.g. a misconfigured `max_completion_tokens`) hits the
  same wall.
...
def decide_scenario(client, trial_pool: list[dict], cfg: Settings, budget: "SpendBudget") -> ScenarioDecision:
    ...
    return ScenarioDecision(
        ...
        stage_a_survivor_counts={sid: sum(1 for r in probed
                                          if r["passes_failure_bar"]
                                          and "missed_obligation" in r["evidence_modes"])
                                 for sid, probed in results.items()},
...
def emit_template_config(cleared_records: list["ClearedRecord"], decision: "ScenarioDecision") -> "TemplateConfigBundle":
    ...
    7. VALIDATE before emitting: assert trigger["id"] in
       ...
       fails.
    8. Render each target constant into its owning .ts file as literal source text
...
Both callers use it verbatim: §12's `stageAScorer` calls
`runJudge([asJudgeObligation(groundTruth)], output.text)`, and §9b's `runVerdict` is a
one-line delegation. **The Stage A eval and runtime enforcement therefore follow the same
...
| `evals/scorers.ts` | `scoreCitation`, `scoreComplianceDate`, `scoreMissedObligation` (TS ports of §4, identical signatures/outcomes to the Python versions), `stageBRecords(clearedSet): ClearedRecord[]`, `partitionForGuardedEval(clearedSet): GuardedPartition` (§12 — deterministic, zero API calls; splits the set into `scored`/`crowdedOut`/`knowledgeOnly` so each record is only held to the expectation its own evidence licenses), `buildGuardedDataset(records): EvalItem[]` (takes `partition.scored`, never the whole set), `stageAScorer`, `stageBScorer`, `guardedCatchScorer`, `runBaselineEval(clearedSet, partition)`, `runGuardedEval(partition: GuardedPartition)`, `runScoreboard(clearedSet?): Promise<ScoreboardResult>` (§12 — computes the partition ONCE and hands the same object to both sides, which is what makes the headline's "identical population" structural rather than a claim). Does **not** define `runJudge` — it imports it from `judge/callJudge.ts`, the single call path (§8) | `@mastra/core/evals`, `agents/baselineAgent.ts`/`guardedAgent.ts`, `judge/callJudge.ts`, `judge/contract.ts`, `schema.ts` (`predictsStageAViolation`), `firmProfile.ts`, `tools/narrowObligations.ts` (`narrowObligationsPure`) | via agent calls (Stage A/B/Judge/guarded generate) |
  ...
  execute: async ({ inputData, mastra, requestContext }) => {
    ...
    const buildBlockedResult = (reason: string, processorId: string, metadata: unknown) => {
      ...
      if (typeof blockedDraft !== "string" || !record
          || !Array.isArray(violatedIds) || violatedIds.length === 0) {
        ...
        throw new Error(`CarverGuardrail tripwire fired but metadata is incomplete `
          + `(blocked_draft=${typeof blockedDraft}, record=${!!record}, `
          + `violated_obligation_ids=${Array.isArray(violatedIds) ? violatedIds.length : typeof violatedIds}) `
          + `— refusing to build an invalid blocked result`);
      }
      return { blocked: true as const, text: null, blocked_draft: blockedDraft, reason, processorId,
    ...
    try {
      ...
      return { blocked: false as const, text: result.text, blocked_draft: null, reason: null,
...
| `obligation_cases` | `{record, scenario, judge_result, expected_outcome, expected_is_failure}` | `test_scoring.py` → `score_missed_obligation` | `scorers.test.ts` → `scoreMissedObligation` |
...
type EvalItem = { input: string; groundTruth: ClearedRecord };

const stageAItems = (records: ClearedRecord[]): EvalItem[] =>
  records.map(r => ({ input: buildStageAPrompt(r), groundTruth: r }));
const stageBItems = (records: ClearedRecord[]): EvalItem[] =>
  records.map(r => ({ input: buildStageBPrompt(r), groundTruth: r }));
...
// THREE named populations, THREE separate runEvals calls, THREE separately-reported
// rates. They are never merged into one "baseline rate" — see the note below.
export async function runBaselineEval(clearedSet: ClearedRecord[], partition: GuardedPartition) {
  const sb = stageBRecords(clearedSet);
  return {
    // THE PAIRED HEADLINE — exactly `partition.scored`: the same records runGuardedEval
    // scores, nothing else. This is the only baseline number ever placed beside the
    // guarded catch rate.
    pairedStageA: partition.scored.length
      ? await runEvals({ target: baselineAgent, data: stageAItems(partition.scored), scorers: [stageAScorer] })
      : null,
    // Reported separately. These records carry real Stage A evidence and their drafts are
    // worth measuring, but the guardrail is never asked about them (§9a — they rank
    // outside their own profile's top 5), so there is no guarded counterpart to pair with.
    crowdedOutStageA: partition.crowdedOut.length
      ? await runEvals({ target: baselineAgent, data: stageAItems(partition.crowdedOut), scorers: [stageAScorer] })
      : null,
    // Reported separately. A DIFFERENT question ("does the baseline know this citation and
    // date?") with no guarded counterpart at all — the guardrail reviews drafts, it does
    // not answer knowledge questions. ONE schema applied to the whole call, so every
    // item's automatic target call returns the structured result the scorer needs.
    stageB: sb.length
      ? await runEvals({ target: baselineAgent, data: stageBItems(sb), scorers: [stageBScorer],
                         targetOptions: { output: StageBResponseSchema } })
      : null,
  };
}
```

> **Why three rates and not one.** The previous draft aggregated Stage A and Stage B items into
> a single baseline failure rate and printed it beside the guarded catch rate as though the two
> were a paired contrast. They were not: the baseline number mixed two different questions over
> two different populations, and its Stage A half included `crowdedOut` records the guarded side
> never evaluated. A reader comparing the two columns would have been comparing different sets
> of records — the exact error §12 exists to prevent, reproduced one level up in the reporting.
> Pairing is only meaningful over an identical population, so exactly one row is paired, and the
> other two are labelled as what they are: separate measurements, each with an em-dash where a
> guarded counterpart would go.

### Guarded path — evidence-specific expectations, not one expectation for every record

**Why the previous draft's guarded eval was invalid.** It mapped the **whole** cleared set
through `buildStageAPrompt` and scored `1` only when the guardrail tripwired on that exact
record — i.e. it required a Stage A block for *every* cleared record, including records
admitted purely for `citation_fabricated`/`date_wrong`. Those records' evidence proves a Stage
B **knowledge** failure and says nothing about whether a draft violates the obligation (§5).
The `>= 0.9` threshold could therefore fail while every component behaved exactly as its own
curated evidence predicted — a test that punishes the system for the test's own category
error. Passing `firmProfileForRecord(record)` does not rescue it either: §9a proves that
profile makes the record *relevant*, not that it wins one of the five ranked slots.

The fix is to ask each record only for what its evidence licenses, via three **explicit,
disjoint, all-reported** partitions:

```typescript
export type GuardedPartition = {
  scored: ClearedRecord[];        // Stage-A-predicting AND narrowable -> MUST block. The headline metric.
  crowdedOut: ClearedRecord[];    // Stage-A-predicting but ranks outside its own profile's top 5 -> reported, not scored
  knowledgeOnly: ClearedRecord[]; // citation/date evidence only -> reported, never expected to block
};

export function partitionForGuardedEval(clearedSet: ClearedRecord[]): GuardedPartition {
  const stageA = clearedSet.filter(predictsStageAViolation);        // §5 — the ONLY licence for a "must block" expectation
  // Deterministic and FREE: narrowObligationsPure is pure array work (§9a), so the
  // partition costs zero API calls and is identical on every run.
  //
  // PRECONDITION: `clearedSet` MUST be the same set CarverGuardrail narrows against
  // at runtime (the vendored src/data/cleared-set.json). The processor imports that
  // set directly (§9a) rather than receiving it, so partitioning a *subset* here
  // would compute `narrows` against a smaller, easier field than the one the
  // guardrail actually ranks over — quietly mis-sorting records between `scored` and
  // `crowdedOut`. runScoreboard's `clearedSet = vendoredClearedSet` default is what
  // makes this hold for every real run; fixture-set callers (§14) assert only on the
  // partition itself, never on live guardrail behavior, so the divergence cannot
  // reach a scored result.
  const narrows = (r: ClearedRecord) =>
    narrowObligationsPure(firmProfileForRecord(r), clearedSet).includes(r.id);
  return {
    scored: stageA.filter(narrows),
    crowdedOut: stageA.filter(r => !narrows(r)),
    knowledgeOnly: clearedSet.filter(r => !predictsStageAViolation(r)),
  };
}
...
function buildGuardedDataset(records: ClearedRecord[]) {
  return records.map(record => ({
    input: buildStageAPrompt(record),
    groundTruth: record,
    requestContext: { firmProfile: firmProfileForRecord(record) },
    // firmProfileForRecord (§8) synthesizes a profile that PROVABLY satisfies both of
    // narrowObligationsPure's required predicates for `record` (§9a's proof) — so
    // recall is measured across the cleared set independently of the fixed demo
    // scenario, unlike DEMO_FIRM_PROFILE which only matches the one scenario the demo
    // is built around. Callers pass partition.scored, NEVER the whole set: membership
    // in `scored` is precisely what makes "this must block" a legitimate expectation.
  }));
}
...
const guardedCatchScorer = createScorer({
  ...
  run: async ({ groundTruth, output }) => {
    ...
    // record. The processor narrows to five candidates and can validly judge several of
    // them violated by one draft; only one can occupy `metadata.record`. Scoring on
    // `record.id === groundTruth.id` therefore recorded a MISS whenever the draft
    // violated the ground-truth obligation *and* a higher-ranked one — punishing the
    // guardrail for finding MORE than expected, which is the opposite of a miss.
    // violated_obligation_ids is the deterministic complete finding, so this asks the
    // question the metric is actually about: was the known obligation caught?
    const caught = !!output.tripwire
      && (output.tripwire.metadata?.violated_obligation_ids ?? []).includes(groundTruth.id);
    return { score: caught ? 1 : 0,
             metadata: { tripwire: output.tripwire ?? null,
                         violated_obligation_ids: output.tripwire?.metadata?.violated_obligation_ids ?? [],
                         display_record_id: output.tripwire?.metadata?.record?.id ?? null } };
  },
});

// Takes the PARTITION, not the cleared set: runScoreboard computes it once and hands the
// same object to both sides, which is what makes "identical population" a structural
// fact rather than a claim two functions independently have to get right.
export async function runGuardedEval(partition: GuardedPartition) {
  // Only `scored` is ever sent to the model. knowledgeOnly/crowdedOut cost 0 calls —
  // running them would spend real money to collect a 0 that proves nothing.
  return partition.scored.length > 0
    ? await runEvals({ target: guardedAgent, data: buildGuardedDataset(partition.scored),
                       scorers: [guardedCatchScorer] })
    : null;   // an empty `scored` is a HARD FAILURE in evals.test.ts, never a silent 100%
}
...
export async function runScoreboard(clearedSet: ClearedRecord[] = vendoredClearedSet): Promise<ScoreboardResult> {
  const partition = partitionForGuardedEval(clearedSet);   // computed ONCE, shared by both sides
  const baseline = await runBaselineEval(clearedSet, partition);
  const guarded = await runGuardedEval(partition);
  return { partition, baseline, guarded };
}
```

**What `npm test` prints** — three rows, one of them paired, each carrying its own `n` so no
denominator is ever implicit:

| Row | Population | n | Baseline | Guarded |
|---|---|---|---|---|
| **Stage A obligation miss — PAIRED** | `partition.scored` | `s` | `>= 0.8` | `>= 0.9` blocked |
| Stage A obligation miss — crowded out | `partition.crowdedOut` | `c` | reported | — (never narrowed, §9a) |
| Stage B citation/date knowledge | `stageBRecords(clearedSet)` | `m` | reported | — (no guarded counterpart) |

Only the first row is a like-for-like contrast, and it is the one that answers goal #14's
question: **for the very same records, the baseline's draft misses the obligation and the
guarded draft is blocked.** The other two rows are real measurements the project should
publish — they are just not comparisons, and the table says so with an em-dash rather than by
...
2. **Paired baseline Stage A rate `>= 0.8`** over `partition.scored` — a live-model tolerance
   band, since re-probing months after curation isn't guaranteed byte-identical (§3).
3. **Guarded catch rate `>= 0.9`** over `partition.scored` — **the same array object** the
   assertion above ran on.
4. **`baseline.pairedStageA.length === guarded.length`** — a structural assertion that the two
   headline numbers really do share a population, rather than two functions independently
   believing they do. Cheap, and it fails loudly if a future refactor re-splits them.

The `crowdedOutStageA` and `stageB` rates are **printed but not asserted at a threshold**: they
...
#11 forbids. `console.table` prints all three rows with their `n`s, plus the `crowdedOut` ids —
so the denominators sit next to the percentages and `scored` can never be mistaken for "the
whole set". This **is** `npm test`'s scoreboard (goal #14: one command, no separate slide), and
it genuinely calls the same
...
above. Let `k` = records satisfying `predictsStageAViolation` (Stage A items, 2 calls each),
`m` = records carrying citation/date evidence (Stage B items, 1 call each; `k + m` may exceed
the set size, since a record can contribute to both), and `s = |partition.scored| <= k` (1
guarded call each). **Total calls = `2k + m + s`.** Worst case at the 200-record ceiling
(`k = m = 200`, every record carrying both kinds of evidence, and every Stage-A-predicting
record narrowable so `s = k = 200`): `2(200) + 200 + 200 = 800` calls — unchanged from the
previous draft's worst case, but no longer *reached* by construction: the guarded term is now
`s`, bounded by `k`, rather than a flat `200` that billed for citation-only records the guarded
side never had a reason to call. Whenever the set contains any knowledge-only or crowded-out
records, `npm test` costs strictly less than before **and** measures something valid, rather
than paying for calls whose only possible outcome was a meaningless `0`. The README states the
actual `k`/`m`/`s` split (read straight off `data/cleared/`'s `baseline_failures` modes and the
deterministic, zero-call partition) and the resulting real cost estimate at ship time, using
the same `price_input_per_million_usd`/`price_output_per_million_usd` rate as `prep` (§13).
...
| `test_imports.py` | **`test_no_circular_imports`** (§1) — walks every `mastra_prep` module with `ast`, extracts intra-package imports **without executing them**, asserts the graph is acyclic and that `budget.py`'s intra-package import set is empty (the leaf property the cycle fix rests on) |
| `test_budget.py` | (all of `budget.py`, §3 — split out of `test_curate.py` alongside the module itself) **`test_ceiling_never_exceeded_even_when_reservation_is_wrong`** — the Case-2 proof: drive spend to just under `_effective_ceiling_usd`, then feed `record_actual` a `usage` reporting `prompt_tokens == MODEL_MAX_CONTEXT_TOKENS` (the worst overage the provider could physically bill) and assert `spend_so_far_usd <= ceiling_usd` **still holds**, `BudgetPoisoned` was raised, and the next `reserve()` raises `BudgetExhausted`; `test_record_actual_normal_case` (worst-case-but-honest usage stays within ceiling, no exception); **`test_effective_ceiling_subtracts_one_max_call`** — `_effective_ceiling_usd == ceiling_usd - MODEL_MAX_CONTEXT_TOKENS * price_in / 1e6`, and a `ceiling_usd` below that margin raises `ValueError` at construction; **`test_reservation_includes_overhead_allowance`** — `reservation_basis_tokens(payload)` equals `len(json.dumps(payload).encode("utf-8")) + REQUEST_OVERHEAD_ALLOWANCE_TOKENS` exactly, and strictly exceeds the payload's own byte count; `SpendBudget(price_in=0.001, price_out=0.001)` raises `ValueError` (the pinned-price floor, enforced independently of `load_settings()`) |
