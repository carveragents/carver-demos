...
**Stage:** 01-spec **Round:** 4 (revision of the previous run's round-5 artifact)
**Status:** Draft
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        report_curation(result)                   # incl. the zero-survivors case (§14)
    finally:
...
const guardedStep = createStep({
  ...
  execute: async ({ inputData, mastra, requestContext }) => {
    ...
    // narrowObligationsPure the processor itself uses). requestContext.firmProfile is
    // guaranteed present on any path that can reach a tripwire: without it narrowing
    // returns zero candidates and §9a passes the draft through unblocked.
    const firmProfile = FirmProfileSchema.parse((requestContext as any)?.firmProfile);
