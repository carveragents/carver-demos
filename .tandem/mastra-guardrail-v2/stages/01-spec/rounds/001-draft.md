...
**Stage:** 01-spec **Round:** 1 (revision of the previous run's round-5 artifact)
**Status:** Draft
...
**What is actually true, and is what this spec now claims.** The bound rests on three
separately-checkable facts, none of which requires knowing the wire format:

1. **Every tokenizable byte of content is inside `payload`.** `system_text`, `user_text`, and
   the JSON schema are the only content-bearing fields a call carries, and `json.dumps`
   renders each of their bytes at least once. So `bytes(json.dumps(payload))` is `>=` the byte
   count of the request's *content*.
2. **A byte is never fewer than a token.** Under any BPE tokenizer with byte-level fallback
   (every modern OpenAI tokenizer has one, precisely so arbitrary input is encodable), no
   input needs *more* than one token per byte. So content tokens `<=` content bytes.
3. **Everything in the gap is covered by a declared, deliberately oversized allowance.**
   `REQUEST_OVERHEAD_ALLOWANCE_TOKENS` (below) covers chat framing, SDK-injected defaults, and
   any structural tokenization not present verbatim in `json.dumps`' output.

Fact 3 is an **allowance, not a proof** — and this spec does not pretend otherwise. What
converts it from an assumption into a guarantee is that it is **verified on every single
call**: `record_actual()` compares the provider's own reported `prompt_tokens` against the
reservation and, on any shortfall, poisons the ledger and hard-stops the run (below). So the
ceiling is enforced by a *checked* bound, not an asserted one. If the allowance were ever
insufficient — a provider framing change, an SDK that adds a large field — the run halts at
the first call that proves it, rather than quietly spending past the ceiling. That is a
strictly stronger safety property than the previous draft's unfalsifiable equality claim.
...
class SpendBudget:
    ...
    def reserve(self, payload: dict) -> float:
        ...
        estimated). Checks spend_so_far + worst_case <= ceiling_usd BEFORE the caller
        is allowed to issue the API call. No additional safety margin is subtracted:
        the margin is already inside the basis, as
        REQUEST_OVERHEAD_ALLOWANCE_TOKENS — and unlike a subtracted margin, that one
        is verified against reality on every call by record_actual(). Raises
        BudgetExhausted if the check fails, OR if the budget has already been poisoned
        by record_actual() (see below) — in the poisoned state, reserve() ALWAYS
        raises, permanently, for the rest of this SpendBudget instance's lifetime.
    ...
    def record_actual(self, reservation: float, usage: dict) -> None:
        ...
        ASSERTS actual <= reservation. **This assertion is what makes the overhead
        allowance a guarantee rather than an assumption** — it is the point where the
        provider's own reported prompt_tokens is checked against what we reserved, on
        every single call. Given the byte-per-token bound, max_completion_tokens'
        API-enforced cap, and REQUEST_OVERHEAD_ALLOWANCE_TOKENS' ~100x headroom over
        real chat framing, a violation means one of those three has genuinely stopped
        holding: a code path reserved against the wrong text, the SDK/provider began
        adding framing larger than the allowance, or OpenAI now bills for something
        not counted here. Each is a real reason to stop, and none is a reason to
        continue on a ledger that is no longer bounded. If it IS violated: this method
        does NOT silently continue. It applies the true-up honestly (the money is already spent —
...
`$88.4 + $0.11 ≈ **$88.5**` — **74% of the $120 ceiling**, so even the pathological case
completes both the trial and the full sweep with real margin.

**What is guaranteed vs. what is sized.** The **guarantee** is unconditional and independent
of every estimate above: `reserve()` refuses any call whose worst case would cross the
ceiling, so the run cannot spend past `total_spend_ceiling_usd`, full stop — that holds for
any ceiling value and any prompt sizes, and it is what rubric 12 requires. The **sizing** (is
$120 comfortable?) rests on the token estimates above, which are estimates and could be wrong.
If they are, the failure mode is safe and loud, not silent: the run stops with
`stop_reason="spend_ceiling"`, reports how many records it probed and how many survived, and
ships the smaller set — exactly goal #11's discipline (report the true number; never pad, and
here, never quietly overspend either). A genuine runaway (e.g. a misconfigured
`max_completion_tokens`) hits the same hard wall.
...
and once in the template (`judge/contract.ts::parseAndValidateVerdicts`, §8 — imported by both
`evals/scorers.ts` and `carverGuardrail.ts`'s verdict stage) — same six steps, same fallback
semantics, tested against the shared `scoring_golden.json` fixture family (§12) on both sides.
The golden fixture family includes an out-of-range-confidence example (`confidence: 5.0` and
`confidence: -0.2`) precisely so step 3's discard-not-clamp behavior is asserted identically
...
Both halves' copies are locked to the same behavior by the shared `scoring_golden.json`
fixture family (§12), which includes a citation-only record (expected `false`), a
missed-obligation record with all three confirmations (`true`), and — the boundary that
matters — a missed-obligation record with one confirmation `False` (`false`).
...
async function runVerdict(draftText: string, candidateIds: string[]): Promise<JudgeResult> {
  ...
  try {
    ...
    // Structured-output parse failure — malformed JSON, a missing field, or (the
    // path added with §4's [0,1] bound) a confidence Zod rejected. Mastra surfaces
    // all of these as a THROW from generate(), not as an inspectable value. §4 step 1
    // already defines the semantics: retry ONCE, then fall back to all-uncertain.
    try {
      const retry = await judgeAgent.generate(prompt, { output: GuardrailVerdictSchema });
      raw = JSON.stringify(retry.object);
    } catch {
      raw = "";   // unparseable by construction -> §4 step 4's fallback for EVERY candidateId
    }
  }
  return parseAndValidateVerdicts(raw, candidateIds);  // §4's shared six-step algorithm — judge/contract.ts
```
...
function buildBaselineDataset(clearedSet: ClearedRecord[]): EvalItem[] {
  ...
  for (const record of clearedSet) {
    // predictsStageAViolation (§5), NOT a bare modes.includes("missed_obligation"):
    // the guarded side selects on exactly this predicate, so using it here too makes
    // the two sides of the scoreboard a PAIRED comparison over an identical record
    // population. (On shipped data the two are equivalent — validate_cleared_record
    // enforces that missed_obligation evidence implies all three confirmations, §5 —
    // so this is about stating the pairing exactly, not about excluding anything.)
...
export async function runGuardedEval(clearedSet: ClearedRecord[]) {
  const partition = partitionForGuardedEval(clearedSet);
  ...
  const result = partition.scored.length > 0
...
1. **`guarded.partition.scored.length >= 1`** — with an explicit failure message
   (`"no cleared record both carries human-confirmed missed_obligation evidence and survives
   narrowing under its own profile; the guarded catch rate would be vacuous"`). A ratio over an
   empty set must never report as a pass; this is the same loud-failure discipline
   `emit_template_config` step 2 applies to the demo (§7), applied to the scoreboard.
2. **Baseline failure rate `>= 0.8`** across whichever of `stageAResult`/`stageBResult` each
   record contributed to — a live-model tolerance band, since re-probing months after curation
   isn't guaranteed byte-identical (§3).
3. **Guarded catch rate `>= 0.9`** over `partition.scored` only.

It prints (`console.table`) baseline-fails% vs. guarded-blocks% side by side, **plus the three
partition sizes and the `crowdedOut` ids** — so the denominators are on screen next to the
percentages and a reader can never mistake `scored` for "the whole set". This **is** `npm
test`'s scoreboard (goal #14: one command, no separate slide), and it genuinely calls the same
