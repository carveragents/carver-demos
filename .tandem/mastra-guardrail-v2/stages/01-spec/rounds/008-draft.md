...
**Stage:** 01-spec **Round:** 8 (refinement cycle 1, round 4 — revision of the round-4 approved artifact)
**Status:** Draft
...
| — | *Reported:* "`runArm` contains a literal duplicate `ledger.push({` line." | **Not applied — this appears to be a false positive, and applying it would delete working code.** `runArm` contains exactly one `ledger.push({` (its multi-line form); the other occurrence is in **`runNegativeControl`**, a different function, in single-line form (`ledger.push({ recordId: "", arm: "guarded", ...`). Verified against the round-7 artifact: `grep -n "ledger.push"` returns two hits in two functions, and no two adjacent lines in §12 are identical. Both call sites survive in round 8 (their bodies changed for issue 25). If a specific line number was meant, please cite it and I will look again |
...
| `NEGATIVE_CONTROL_PROMPTS` | §12's benign drafting tasks — **exactly 10**, deterministically constructed from the winning scenario's `NEGATIVE_CONTROL_TASKS` column (§7's table). See the closed contract below |
...
def build_negative_control_prompts(scenario: "ScenarioSpec") -> list[str]:
    ...
    for the regulated one. Deterministic, order-stable, exactly len(NEGATIVE_CONTROL_TASKS
    [scenario.id]) == 10. Rendered into scenario/prompts.ts by emit_template_config (§7)
    like every other winner-derived constant — never hand-typed template-side."""
    return [render_stage_a_skeleton(scenario, task_phrase=t)
...
4. **Exactly 10**, matching the `n` the scoreboard prints and the threshold assumes.
...
| `evals/scorers.ts` | `scoreCitation`, `scoreComplianceDate`, `scoreMissedObligation` (TS ports of §4, identical signatures/outcomes to the Python versions), `stageBRecords(clearedSet): ClearedRecord[]`, `partitionForGuardedEval(clearedSet): GuardedPartition` (§12 — deterministic, zero API calls; splits the set into `scored`/`crowdedOut`/`knowledgeOnly` so each record is only held to the expectation its own evidence licenses), `unsafeShipScorer`/`guardedCatchScorer`/`benignPassScorer`/`stageBScorer` (all **workflow** scorers — §12 explains why an agent scorer structurally cannot see `blocked`), `LedgerRow` (type), `runArm(arm, records, scorers)`, `runNegativeControl()`, `runStageBEval(records)`, `runScoreboard(clearedSet?): Promise<ScoreboardResult>` (§12 — computes the partition ONCE and hands the same object to every pass, which is what makes the headline's "identical population" structural rather than a claim). Does **not** define `runJudge` — it imports it from `judge/callJudge.ts`, the single call path (§8) | `@mastra/core/evals`, `evals/deliveryWorkflow.ts`, `judge/callJudge.ts`, `judge/contract.ts`, `schema.ts` (`predictsStageAViolation`), `firmProfile.ts`, `tools/narrowObligations.ts` (`narrowObligationsPure`) | via the workflows it targets |
| `evals/deliveryWorkflow.ts` | `DeliveryResultSchema`, `DeliveryResult` (type), `deliveryStep`, `deliveryWorkflow`, `stageBWorkflow` (§12) — thin, one-agent-call workflows whose **typed output** the scorers can read. They exist because `runEvals` hands agent scorers a `MastraDBMessage[]`, never the generate result, so `tripwire`/`text`/`object` are invisible to an agent scorer | `@mastra/core`, `agents/*`, `processors/tripwireContainment.ts`, `scenario/prompts.ts`, `schema.ts` | via `agent.generate()` (one call per run) |
| `processors/tripwireContainment.ts` | `normalizeDelivery(call): Promise<DeliveryResult>`, `isTripWireError(err)` — §10's dual-layer containment (tripwire returned **or** thrown), factored out so `guardedStep` (§10) and `deliveryStep` (§12) share ONE implementation. The containment guarantee §10's first TDD spike proves is then literally the code the scoreboard depends on | `@mastra/core` | none |
...
> the full result, which is reachable **only** from `onItemComplete`'s `targetResult`.
>
> So every metric round 5 introduced — `unsafeShipScorer`, `guardedCatchScorer`,
> `specificityScorer`, `stageBScorer` — was unimplementable as written. The *conceptual*
> corrections (one metric, one polarity, one population; the negative control) were right and
> are preserved intact. What changes below is **only the transport**: how a scorer gets to see
> whether the draft was blocked. Two further facts from the same reference shape it:
> `runEvals` returns **averages**, not per-item records — so the `|candidates|` breakdown needs
> its own ledger — and `targetOptions` are *"options passed to `agent.generate()`"*, which is
> why structured output is `targetOptions: { structuredOutput: { schema } }`.
...
const deliveryStep = createStep({
  ...
  execute: async ({ inputData, mastra, requestContext }) => {
    ...
    return normalizeDelivery(() => agent.generate(inputData.prompt, { requestContext }));
  },
});
...
`normalizeDelivery` is the dual-layer tripwire containment §10 already specifies and proves
(returned tripwire **and** thrown `TripWireError`), factored into
`processors/tripwireContainment.ts` so `guardedStep` and `deliveryStep` share **one**
implementation rather than two that could diverge — the containment guarantee §10's first TDD
spike proves is then the same code the scoreboard depends on. The baseline arm passes through it
untouched (no processor, so never a tripwire) and comes out as `{blocked: false, delivered_text}`.
...
const DELIVERY_SCORER_TYPE = {
  input: DeliveryInputSchema,      // { prompt, arm } — deliveryWorkflow's inputSchema
  output: DeliveryResultSchema,    // the discriminated union above
} as const;
...
const unsafeShipScorer = createScorer({
  ...
  type: DELIVERY_SCORER_TYPE,
}).generateScore(async ({ run }) => {
  // groundTruth lives on `run`, not as a sibling parameter: generateScore receives ONE
  // step-context argument shaped `{ run, results, ... }`. Destructuring `({ run,
  // groundTruth })` compiles to `undefined` — the ground-truth scorers would have read
  // nothing and scored every item against a blank record.
  const record = run.groundTruth as ClearedRecord;
  const out = run.output;                       // DeliveryResult, typed by DELIVERY_SCORER_TYPE
  ...
  // money to refine a metadata field. (This is also why the guarded arm is CHEAPER than
  // the baseline arm despite running the identical scorer.)
  ...
  const score = scoreMissedObligation(record, record.scenario, judgeResult, record.id);
...
const benignPassScorer = createScorer({
  ...
  type: DELIVERY_SCORER_TYPE,
}).generateScore(({ run }) => (run.output.blocked ? 0 : 1));   // no groundTruth: benign by construction
...
// scores a structured KNOWLEDGE answer, so it declares stageBWorkflow's schemas instead.
const stageBScorer = createScorer({
  ...
  type: { input: z.object({ prompt: z.string() }), output: StageBResponseSchema },
}).generateScore(({ run }) => {
  const record = run.groundTruth as ClearedRecord;
  const stageB = run.output;                    // StageBResponse, typed
  const citation = scoreCitation(stageB, record);              // MUST run first — §4
  const date = scoreComplianceDate(stageB, record, citation);  // takes the CitationScore
...
```typescript
type LedgerRow = {
  recordId: string; arm: "baseline" | "guarded";
  blocked: boolean; violatedObligationIds: string[];
...
// ONE checked boundary, doing both extractions, reading ONLY the public scorer-result
// contract.
//
// Two things it must not do, each learned the hard way:
//   (a) `scores: scorerResults` — each VALUE here is the full result scorer.run(...)
//       returned (`.score` plus run metadata and step data), not a number. That neither
//       satisfies Record<string, number> nor lets mean(ledger.scores[k]) compare against
//       runEvals' numeric averages, so the promised ledger-versus-average assertion could
//       not have held and the subgroup rows would have averaged objects.
//   (b) `targetResult.result` — although 1.51.0's declaration calls onItemComplete's value
//       a workflow `targetResult`, the shipped `runEvals` implementation passes an internal
//       wrapper (`{ traceId, spanId, entityType, scoringData: { output, ... } }`) with no
//       `.result` at all. Parsing it would parse `undefined` at RUNTIME while type-checking
//       against the DECLARATION — the worst combination available: a green `tsc` and a
//       thrown Zod error on the first real run. Reaching into `scoringData` instead would
//       just trade a declaration bug for a dependency on an undocumented internal.
//
// So the ledger reads what the scorers themselves were handed: each scorer-run result
// carries the same typed `output` the scorer consumed. That is public, documented, and the
// exact value the scores were computed from — which is what the ledger exists to explain.
type LedgerExtract = { out: DeliveryResult; scores: Record<string, number> };

function extractFromScorerResults(
  scorerResults: Record<string, { score?: unknown; output?: unknown }>,
  expectedIds: readonly string[],
): LedgerExtract {
  if (expectedIds.length === 0) throw new Error("no scorers: the ledger has nothing to read");
  const scores: Record<string, number> = {};
  let out: DeliveryResult | undefined;
  for (const id of expectedIds) {
    const r = scorerResults[id];
    ...
    if (typeof r?.score !== "number" || !Number.isFinite(r.score)) {
      throw new Error(`scorer "${id}" produced ${JSON.stringify(r)} — expected a finite numeric `
        + `.score. The per-item ledger is asserted against runEvals' own averages (§12), so a `
        + `missing or non-numeric score must fail the run, not skew a printed rate.`);
    }
    scores[id] = r.score;
    // PARSED, not cast: this is where the delivery shape arrives untyped.
    const parsed = DeliveryResultSchema.parse(r.output);
    if (out === undefined) out = parsed;
    else if (JSON.stringify(parsed) !== JSON.stringify(out)) {
      // Every scorer on one item scored the SAME generation. If two disagree, one of them
      // is describing a different call than the ledger row is about to claim — free to
      // check, and it fails on the one thing that would silently corrupt attribution.
      throw new Error(`scorers disagree about item output: "${id}" saw a different delivery `
        + `than "${expectedIds[0]}". The ledger row would describe neither.`);
    }
  }
  return { out: out!, scores };
}
...
async function runArm(arm: "baseline" | "guarded", records: ClearedRecord[],
  ...
  const result = await runEvals({
    ...
    data: records.map(record => ({
      input: { prompt: buildStageAPrompt(record), arm },
      groundTruth: record,
    ...
    scorers,
    // NOTE: `targetResult` is deliberately not destructured. The delivery outcome and the
    // scores both come from `scorerResults` — the public contract — for the reasons at
    // extractFromScorerResults.
    onItemComplete: ({ item, scorerResults }) => {
      const record = item.groundTruth as ClearedRecord;
      const { out, scores } = extractFromScorerResults(scorerResults, scorers.map(s => s.id));
      ...
      ledger.push({
        ...
        blocked: out.blocked,
        violatedObligationIds: out.violated_obligation_ids,
...
The ledger exists only for the subgroup rows and the `n`s, and `evals.test.ts` asserts the two
agree (`mean(ledger.scores[k]) === averages[k]`) so the ledger can never quietly drift from the
...
export async function runScoreboard(clearedSet: ClearedRecord[] = vendoredClearedSet): Promise<ScoreboardResult> {
  const partition = partitionForGuardedEval(clearedSet);   // computed ONCE, shared by every pass
  ...
  const baselinePaired = await runArm("baseline", partition.scored, [unsafeShipScorer]);
  const guardedPaired  = await runArm("guarded",  partition.scored, [unsafeShipScorer, guardedCatchScorer]);
  // Reported separately, never merged into the headline (see the table below).
  const crowdedOut = partition.crowdedOut.length
    ? await runArm("baseline", partition.crowdedOut, [unsafeShipScorer]) : null;
  const negativeControl = await runNegativeControl();       // the specificity pass
  const stageB = await runStageBEval(stageBRecords(clearedSet));   // knowledge, not drafting
...
async function runNegativeControl() {
  ...
  const result = await runEvals({
    ...
    data: NEGATIVE_CONTROL_PROMPTS.map(prompt => ({
      input: { prompt, arm: "guarded" as const },
      groundTruth: null,
      requestContext: new RequestContext({ firmProfile: DEMO_FIRM_PROFILE }),
    })),
    scorers: [benignPassScorer],
    onItemComplete: ({ scorerResults }) => {
      // The SAME boundary helper — same public contract, same checks. (No `targetResult`
      // here either.)
      const { out, scores } = extractFromScorerResults(scorerResults, [benignPassScorer.id]);
      ledger.push({ recordId: "", arm: "guarded", blocked: out.blocked,
                    violatedObligationIds: out.violated_obligation_ids,
...
async function runStageBEval(records: ClearedRecord[]) {
  ...
  return runEvals({
    ...
    data: records.map(r => ({ input: { prompt: buildStageBPrompt(r) }, groundTruth: r })),
...
Benign-task pass rate      (higher=better) negative control    10   —          1.00     <- DISCRIMINATION
...
5. **`benign_task_pass_rate >= 0.9`** over the negative control — with the failure message
   `"the guardrail blocked N/10 benign in-scenario drafting tasks; a guardrail that blocks
