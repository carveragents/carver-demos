...
**Stage:** 01-spec · **Status:** draft for review (round 2 — refinement cycle 1)
...
> was rebuilt as a **category-stratified** snapshot with a **catalog-sourced** category map. This
> revision updates §2.1 (field name), §2.2/§2.3/§8 (the new stratified foundation + refreshed
> coverage numbers), and §4.2/§11 (catalog-sourced category). All other design — module
> boundaries, schema, view inventories, deterministic formulas, testability — is unchanged.
...
- **Scope honesty (load-bearing).** The snapshot is a **category-stratified** sample — *not* a
  uniform random draw and *not* a contiguous offset slice. It spans **405 of the ~1,087** corpus
  topics. Therefore:
  ...
  - Both apps show a persistent **sampling-caveat banner** on their overview stating the stratified
    ...
    coverage (405/1,087), and that the sample is category-stratified, not random. This is part of
    the honest-coverage stance (§8) and is non-negotiable.
...
| `ingest.py` | (a) one-time `pull_snapshot(...)` over the Artifacts API (incl. the per-category `topic_ids_in` pulls and the catalog category map); (b) `load_snapshot(path) -> Iterator[dict]` streaming raw envelopes from the JSONL. | `pull_snapshot(api_key, dag, out_path, page_size, topic_ids=None) -> int`; `load_snapshot(path: Path) -> Iterator[dict]` |
...
| `load.py` | App-facing cached loader: build-or-load the parquet from the JSONL (+ category map), return the frame. The only thing the apps import to get data. | `load_normalized(parquet_path, jsonl_path, categories_path) -> DataFrame` (wrapped in `st.cache_data` at the app edge) |
...
5. No live API, ever, at steps 1–5 except the one-time offline pull that created the JSONL and the
   category map.
...
`acronym`) persisted alongside in `topic_categories.csv`; these are an **available** input that MAY
enrich range views and enable a Cockpit cross-check (annotation `jurisdiction_country` vs topic
`jurisdiction_code`) — noted as optional, not a required v1 build.
...
|---|---|---|---|
| 0 | **Overview & "What is an annotation" explainer** | the pitch | Plain-language explainer of the annotation object with one annotated real example; **headline KPIs** (annotations ≈59K, the 3-category split — Finance / Data protection / Medical Devices, topics 405, countries 111, regulators, update_types, % with full score trio, median richness); the **sampling-caveat banner** (§2.2). |
...
**QA action:** see at a glance which fields/slices to prioritize.
...
- **Scope & stratification are labelled.** The sampling-caveat banner (category-stratified sample:
  ...
  in full, Finance sub-sampled; 405/1,087 topics; not random) appears on both apps; corpus-wide
  breadth ("1,087 topics, 241 jurisdictions") is clearly marked as describing the full dataset vs.
  the snapshot's live metrics.
...
- `ingest.pull_snapshot`: pagination/termination + `topic_ids_in` filtering and the catalog
  category fetch, with **httpx stubbed** (no network).
...
- **A3** `category` is catalog-sourced via `GET /api/v1/feeds/categories/{id}/topics`
  ...
  (§4.2). Optional topic attributes from the same catalog rows (`jurisdiction_code`, `scope`,
  `sectors`, `industries`, `govt_body`, `acronym`) may enrich range views / a Cockpit cross-check.
