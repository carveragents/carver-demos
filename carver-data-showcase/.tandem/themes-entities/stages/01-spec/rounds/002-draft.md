...
- **`classify_entities` (stubbed client):** the **request-builder** (deterministic
  chunking into ~50, `custom_id` format, request-body shape, taxonomy present); the
  **response-parser** (maps `custom_id` → entities; validates the schema); and the
  **detect → retry → fall back** policy (§4.3) — a stub returning a short/malformed chunk
  triggers the bounded retry (assert the retry is attempted), and a stub that stays
  malformed through `MAX_RETRIES` yields the explicit `type="Other"` fallback. The
  `--sample` synchronous path is exercised with the same stubbed client.
