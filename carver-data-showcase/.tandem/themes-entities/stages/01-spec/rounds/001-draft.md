...
- **Output:** `data/entity_mentions.csv` (`entity,count`) and `data/tag_mentions.csv`
  (`tag,count`), each sorted by descending count.
...
| `count`  | int | number of records that mention it (mentions) |

Example: `European Central Bank,18223`

### 3.2 `data/tag_mentions.csv`  *(written by extract; read by rollup)*
| column | type | meaning |
|---|---|---|
| `tag`   | str | raw tag string |
| `count` | int | mentions across the corpus |

Example: `anti-money laundering,9120`
...
or split buckets. Examples below are real corpus entities where named in the verified
facts; buckets the regulator-skewed top list under-populates are anchored with bodies
that unambiguously appear in a global regulatory corpus (their definitive per-bucket
exemplars are confirmed during `--sample` prompt iteration in Stage 02).

| Bucket | One-line definition | Examples |
|---|---|---|
| **`Regulator / Supervisor`** | A body that regulates/supervises a sector, **including central banks**. | SEC, ESMA, FCA, FDIC, EBA, MFSA (Malta Financial Services Authority), ECB & Deutsche Bundesbank (central banks) |
| **`Government body`** | A government organ that is not primarily a financial supervisor: ministries, legislatures, courts, law-enforcement. | European Parliament (legislature), U.S. Department of the Treasury (ministry), Department of Justice (law-enforcement) |
| **`International body`** | Intergovernmental and standard-setting organisations. | IMF, European Commission, Basel Committee / FATF / IOSCO (standard-setters) |
| **`Company`** | A commercial firm / private-sector organisation. | Mastercard, CIBanco |
| **`Person`** | A named individual (official, executive, etc.). | Christopher P. Buttigieg |
| **`Other`** | Places, and anything genuinely unclassifiable. | Bolivia, Croatia, garbled/unknown strings |
...
- **Unknown / garbled entities:** instructed to still emit a row, `type="Other"`,
  `canonical_name=entity` (never dropped — every input gets a row).

### 4.3 Response validation & retry (per chunk)
A chunk's response is **malformed** if any of: not valid JSON; fewer objects than entities
sent; an object missing `entity`/`type`/`canonical_name`; or a `type` not in the 6 buckets.
Detection is per chunk via the stable `custom_id` (§4.4). Handling:
- Any **individual** entity whose object is missing/invalid → defaulted to
  `type="Other", canonical_name=entity` (so the corpus is always fully covered).
- A **whole-chunk** failure (unparseable / empty) → that chunk's entities are collected
  and re-classified via a **bounded retry** through the synchronous `--sample` path
  (`MAX_RETRIES`, small); entities still unresolved after retries default to `Other`.
This is unit-tested with a **stubbed client** returning malformed payloads — no network.

### 4.4 Batch mechanics (ONE job, sharded — not one giant call)
1. **Build** a single request file `data/entity_batch_requests.jsonl`: one line **per
   chunk** of `ENTITY_CHUNK_SIZE` (~50) entities. Each line is a Batch request envelope
   `{custom_id, method:"POST", url:"/v1/chat/completions", body:{model, messages, ...}}`
   with `custom_id = f"chunk-{i:05d}"` (stable map back to that chunk's exact entity list,
   held in memory / re-derivable from the deterministic chunking).
2. **Submit ONE Batch job:** upload the file via the Files API (`purpose="batch"`), then
   create the batch against endpoint `/v1/chat/completions`, `completion_window="24h"`.
3. **Poll** the batch status until `completed` (or `failed`/`expired` → surface + stop).
4. **Fetch + parse** the output file; for each line, look up `custom_id` → the chunk's
   entity list, validate (§4.3), and collect `{entity,type,canonical_name}` rows.
5. **Map back & write/merge** into `entity_types.csv`.
...
- **Idempotency:** the corpus is static, so the mapping is built **once**. On any re-run,
  `classify_entities` classifies only entities **absent** from `entity_types.csv` (set
  difference on the `entity` column); an already-complete cache → no Batch job submitted.
...
### 5.4 Deck slide (8 → 9)
- A new `_slide_themes_entities(c, ctx, df, catalog_df)` built from the **same** three
...
- `_build_context` is extended to carry the term-stat numbers (distinct counts, totals,
  medians, top buckets) so the slide's callout text matches its charts.
- **Graceful absence:** `build_deck` composes its **active slide list** at runtime —
  the themes slide is included only when `load_term_stats()` returns data; otherwise the
  deck builds with the original 8 slides. (The existing `tests/test_deck.py` page-count
  assertion is updated in Stage 02 to count the active slide list rather than a fixed
  `len(SLIDES)`.)
...
- **`classify_entities` (stubbed client):** the **request-builder** (deterministic
  chunking into ~50, `custom_id` format, request-body shape, taxonomy present) and the
  **response-parser** (maps `custom_id` → entities; validates the schema; the
  **malformed/short/unknown-type → `Other`** path); the `--sample` synchronous path with a
  stubbed client.
